...
extern "C" {  // prevents func-names mangling

void ethernet(char *buff)
{
    WSADATA WinsockData;
    HINSTANCE m_hInst;
    ...
    AsnObjectIdentifier MIB_ifMACEntAddr = { 
        sizeof(OID_ipMACEntAddr) / sizeof(UINT),0
    };
    AsnObjectIdentifier MIB_ifEntryType = {
        sizeof(OID_ifEntryType) / sizeof(UINT),0
    };
    AsnObjectIdentifier MIB_ifEntryNum = {
        sizeof(OID_ifEntryNum) / sizeof(UINT),0
    };
    ...
    if (WSAStartup(MAKEWORD(2, 0), &WinsockData) != 0) {
        fprintf(stderr, "No Winsock 2.x installed!\n");
        return;
    }
                ...
                if ((!ret)
                    ...
                    printf("MAC Address of interface #%i: %s\n", j,
                           buff);}
                               ...
                               be examined */
//    getch();
    for (i=0;i<20;i++) { //To uppercase
	 int c=buff[i];
   	 if (islower(c)) buff[i]=toupper(c);
    }
