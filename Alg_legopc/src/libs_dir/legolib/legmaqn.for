...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      SUBROUTINE LEGMAQN(NEQSIS,IPCJ,ICJ,NBL,ISLB,NUSTA,NUSCI,
     ...
     $   IGRAD,ICN,IRN,IKEEP,IW,W,RHS,MXTER,IQNMX,QN1,QN2,QN,SS,
     $   tresi,tjaco,tma28a,tccc,nresi,njaco,naaa,nccc,npas,stampa1,
...
C       - VETTORI PER QUASI-NEWTON  QN1,QN2,QN,SS

      DATA BETA/0.5/KBEMX/5/
      DATA NITERJ/20/NJACMX/1/ !niterj=7,10
...
C       tt=etime(tmp)-tt                               
        ...
        IF((ICONVE.EQ.0).OR.(ITERT.EQ.0))THEN
          ...
          IF((JACYES.EQ.0).AND.(ITERT.GT.0))THEN
            IF((VAR.LT.VARR).AND.(ITER.GE.NITERJ))THEN
              JACYES=1 !superato numero max iterazioni, calcolare lo jacobiano
            ELSEIF(VAR.GE.VARR)THEN !difficoltà di convergenza
              CALL SSWTCH(4,LL)
              IF(LL.EQ.1)WRITE(6,1000)VAR,VARR 
              IF(KBETA.LT.KBEMX)THEN  !correzioni delle soluzioni con beta
                KBETA=KBETA+1
...
C             tt=etime(tmp)-tt                   
          ...
          IF((ITER.EQ.0).OR.((VAR.LT.VARR).AND.(ITER.LT.NITERJ)))THEN 
            IF((ITERT.EQ.0).OR.(VAR.LT.VARR))THEN
              CALL SCOPY(NEQSIS,XY,1,XYO,1)  ! copia xy in xyo
              CALL SCOPY(NEQSIS,RN,1,RNO,1)  ! copia rn in rno
              VARR=VAR                                
            ENDIF
...
C           tt=etime(tmp)                           
            ...
            CALL QUANEW(NQSISV,IQNMX,QN1,QN2,QN,RHS,SS,ITER,JOK,KBETA,
     $              KQN,stampa1) !correzione della soluzione con quasi-newton
C           tt=etime(tmp)-tt                       
            tccc=tccc+tt                            
            nccc=nccc+1                             
            if(stampa1.eq.1)write(55,*)'tempo qn-ma kqn',tt,kqn
            KBETA=0         
            CALL SAXPY(NQSISV,1.,SS,1,XYV,1)  ! xyv=xyv+ss aggiornamento
