...
//#endif	
#ifdef CLIMON
	#define S_FUNCTION_NAME  LegoCliMON
#endif	
...
// ATTENZIONE !!!!!!!!!  con legoclisinc deve essere sempre = 1 !! impongo 1
    imodel=1;
...
//    leggiSimul();
// PROVVISORIO imposto i valori che si dovranno leggere eventualmente da file
num_ing=0;
num:usc=2;
...
//	LegoClient( INIT);
	if(primavolta==0) {
		primavolta=1;
// Apro shared memory e event, calcolo dei puntatori alle variabili XY e UU
		CurDirLength=GetCurrentDirectory( CurDirLengthbuf, CurDir);
 		SetCurrentDirectory(modelPath);
//GUAG!
// 		if(retexec <= 31) retexec=WinExec("LgSincro /SINC",SW_SHOWNORMAL);
	retget=FALSE;
	
// non lancio nessuna task
//	if(Pi.hProcess != NULL) {
//		retget=GetExitCodeProcess(Pi.hProcess, dw);
//		if( retget && (*dw != STILL_ACTIVE) ) CloseHandle(Pi.hProcess);
//	}
////	if(!retget) MessageBox( NULL, "GetExitCodeProcess errata", "attenzione", MB_OK|MB_ICONINFORMATION );
////	MessageBox( NULL, "sono qui: dopo getexit", "attenzione", MB_OK|MB_ICONINFORMATION );
//	
//	if( *dw != STILL_ACTIVE){ // se il processo lgsincro non c'è lo lancio...
...
//		Si.cbReserved2 = 0; 
//	
////	MessageBox( NULL, "lancio lgisncro", "attenzione", MB_OK|MB_ICONINFORMATION );
//		if( !CreateProcess( NULL,(LPTSTR) "LgSincro.exe /SINC",NULL,NULL,FALSE,DETACHED_PROCESS , 
//	   	NULL,NULL,&Si,&Pi))
//	   	{	
//		    ssSetStopRequested(S,1); 
//		    ssSetErrorStatus(S,"LEGO Error: LGSINCRO not started");
//			return;
//		} 
//		else
//		{ 
//printf("process:   %x  thread: %x\n",Pi.hProcess,Pi.hThread);
...
//	}
//	free(dw);
//	retget=GetExitCodeProcess(Pi.hThread, dw);
//        if((retget !=0) && (*dw == STILL_ACTIVE)) MessageBox( NULL, "processo lgsincro attivo", "attenzione", MB_OK|MB_ICONINFORMATION );
//
...
//mi sincronizzo solo all'inizializzazione

		while ((hEveSINIT = OpenEvent( EVENT_ALL_ACCESS  ,TRUE,EVESINIT) ) == NULL);
//GUAG!		retWait = WaitForSingleObject(  hEveSINIT,TASK_TIMEOUT);
		printf("Ho agganciato il semaforo EVESINIT\n");
...
////GUAG! stampa seguente
//		printf("provmap Client: hEveS=%d hEveCSINC=%d\n",hEveS,hEveCSINC);

// miaggancio alla shared mem
		if( (hMem1=OpenMap( (LPSTR) MapName)) == NULL) {
				printf("provmap Client: Shared memory assente\n");
				return;
		}
		pShrMem1 = (MEMORIASHARED *) MapView(hMem1);
// niente		RESTART = 1; // inizializzo il flag di restart a true
//GUAG! stampa seguente
		printf("provmap Client: Memoria shared aperta hMem1=%d \n",hMem1);
...
retWait = WaitForSingleObject(  hEveCSINC,TASK_TIMEOUT);

if(retWait == WAIT_TIMEOUT) {
//	while (retWait == WAIT_TIMEOUT && pShrMem1->control[2] == SFREEZE) 
//        printf("LegoCli: Time OUT!!-------------->  RESTART=%d tempo=%f chiamata=%d\n",RESTART, pShrMem1->tempo,numero_call);
//	printf("LegoCli: Time OUT!!\n");
	while (pShrMem1->control[2] == SFREEZE && retWait == WAIT_TIMEOUT) 
           retWait = WaitForSingleObject(  hEveCSINC,TASK_TIMEOUT); // se lgsincro è in freeze ed è scaduto il timeout ... aspetto
	if(retWait == WAIT_TIMEOUT) {// se lgsincro NON è in freeze ed è scaduto il time out fermo tutto		   
		   ssSetStopRequested(S,1); 
		   if(pShrMem1->control[2] != SSTOP) ssSetErrorStatus(S,"LEGO Error: Task Aborted");
		   return;
	}
}
if(primo_passo==0) hEveCSINCOK = OpenEvent( EVENT_ALL_ACCESS  ,TRUE,EVECSINCOK);
//	printf("LegoCli: dopo map hEveCSINCOK\n");
  
 
//GUAG!  retWait = WaitForSingleObject(  hEveC,TASK_TIMEOUT);
...
//GUAG! stampa seguente
        numero_call++;
//        printf("--------------------------------->  RESTART=%d tempo=%f chiamata=%d\n",RESTART, pShrMem1->tempo,numero_call);
...
//			      pShrMem1->xy[varsc_usc[0].ind]*pShrMem1->cnxy[varsc_usc[0].ind]);		
          ResetEvent(hEveCSINC);
          SetEvent(hEveCSINCOK);
//GUAG!    SetEvent(hEveS);
}
...
//		pShrMem1->control[1] = 1;  // Invio al server il comando di stop
		pShrMem1->control[2] = 1;  // Invio a lgsincro il comando di stop
//		SetEvent(hEveS);
//		SetEvent(hEveC); // libero lgsincro al posto della task lego
                retWait = WaitForSingleObject(  hEveCSINC,TASK_TIMEOUT);
		SetEvent(hEveCSINCOK); //libero lgsincro
		primavolta=0;		
		primo_passo=0;
