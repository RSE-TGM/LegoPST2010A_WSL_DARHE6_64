      SUBROUTINE BTB0I3(IFO,IOB,DEBL)
...
C
      WRITE(IFO,3014)IOB
 3014 FORMAT('FNAC',A4,2X,
     $'--IN-- NaCl MASS FRACTION COMING INTO HEAD LOSS')
C
      WRITE(IFO,3015)IOB
 3015 FORMAT('FINV',A4,2X,
     $'--IN-- NaCl MASS FRACTION FOR FLOW INVERSION')
C
      WRITE(IFO,3016)IOB
 3016 FORMAT('HINV',A4,2X,
     $'--IN-- ENTHALPY AT INLET FOR FLOW INVERSION')
C
      RETURN
      END
      SUBROUTINE BTB0I4(IOB,MOD)
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z)                              !DBLE
      COMMON/BTB000/IBLOC
      CHARACTER*8 IBLOC
      CHARACTER*4 IOB
...
C
      RETURN
      END
      SUBROUTINE BTB0I2(IFUN,VAR,MX1,IV1,IV2,XYU,DATI,ID1,ID2,NOM1,NOM2,
     $ IER,CNXYU,TOL)
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z)                              !DBLE
...
C
 ...
 1010 FORMAT('*   DATA TO BE ASSIGNED TO DEFINE THE HEAD LOSS')
      WRITE(14,1020)WL1,WL2,PM1,PM2,PV1,PV2,HM1,HM2,
     $             'FR.N','aCl ',CS1,CS2
      WRITE(14,1011)
...
C
  ...
  502 FORMAT(14X,A)
      ...
      END
C
      SUBROUTINE BTB0C1(IFUN,AJAC,MX5,IXYU,XYU,IPD,DATI,RN,NOM1,NOM2)
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z)                              !DBLE
...
C
   42 AJAC(1,1) = FC*RK0/RO*(ABS(W)+.1/(2*Q0))
      AJAC(1,2) = -1.
      AJAC(1,3) = 1.
...
C
  46  AJAC(1,1) = -1.
      AJAC(1,2) = FC*RK0/RO*(ABS(W)+.1/(2*Q0))
      AJAC(1,3) = 1.
      AJAC(1,5) = RK0/RO/2.*(W*ABS(W)+.1/Q0*W)
      GO TO 50
C
...
C
 48   AJAC(1,1) = 1.
      AJAC(1,2) = -1.
      AJAC(1,3) = FC*RK0/RO*(ABS(W)+.1/(2*Q0))
      AJAC(1,5) = RK0/RO/2.*(W*ABS(W)+.1/Q0*W)
 50   RETURN
      END
CC
      SUBROUTINE BTB0D1(BLOCCO,NEQUAZ,NSTATI,NUSCIT,NINGRE,SYMVAR,
