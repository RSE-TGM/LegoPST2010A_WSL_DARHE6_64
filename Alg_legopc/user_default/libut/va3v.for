C**********************************************************************
C 
C              modulo VA3V.FOR - Valvola a 3 vie
C 
C         Nota: e` una versione "ritagliata" di VACT
C 
C**********************************************************************
      SUBROUTINE VA3VI3 (IFO,IOB,DEBL)
...
C
C
      SUBROUTINE VA3VI4 (IOB,MOD,IBLOC,NC)
      CHARACTER*4 IOB, MOD, IBLOC*8, ICA*1
C
ccc
ccc   Modifiche per forzare il modello a 3 vie
ccc
ccc    1 WRITE(6,2)
ccc      READ(5,3,ERR=1)ICA
ccc      NC = ICHAR(ICA) - 48
ccc      IF (NC .LT. 0 .OR. NC .GT. 3) GOTO 1
ccc      IBLOC = MOD(1:3) // ICA // IOB
ccc
      NC = 3
      IBLOC = 'VA3V' // IOB
ccc
...
C
C
      SUBROUTINE VA3VI2 (IFUN,VAR,MX1,IV1,IV2,XYU,DATI,ID1,ID2,
...
ccc   Modifiche per forzare il modello a 3 vie
ccc
ccc      WRITE (BLOCCO, '(A4)') IBL1
ccc      READ (BLOCCO, '(3X,I1)') NC
ccc
      NC = 3
ccc
      DATI (ID2) =  NC
...
C
C
      SUBROUTINE VA3VC1 (IFUN,AJAC,MX5,IXYU,XYU,IPD,DATI,RNI,IBL1,IBL2)
C
...
C
C
      SUBROUTINE VA3VD1 (BLOCCO,NEQUAZ,NSTATI,NUSCIT,NINGRE,SYMVAR,XYU,
