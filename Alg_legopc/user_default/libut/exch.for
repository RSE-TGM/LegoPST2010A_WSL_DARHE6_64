...
C release 5.16
C data 11/3/97
C reserver @(#)exch.f	5.16
C**********************************************************************
C----------------------------------------------------------------------------
C
C        MODULO EXCH versione Puertollano (Migliavacca)
C      
...
C----------------------------------------------------------------------------
C
      SUBROUTINE EXCHI3(IFO,IOB,DEBL)
...
C
 ...
 3001 FORMAT('PU',I2,A4,2X,'--UA-- FLUID PRESSURE', 
   ...
   30 CONTINUE
      WRITE(IFO,3101)IOB
 3101 FORMAT('PUTU',A4,2X,'--UA-- TUBE OUTLET FLUID PRESSURE')
C
      WRITE(IFO,3102)IOB
 3102 FORMAT('WITU',A4,2X,'--UA-- TUBE INLET MASS FLOW')
      DO 40 K=1,NCEL-1
...
C
 ...
 3003 FORMAT('QTI',I1,A4,2X,'--UA-- POWER EXCHANGED BETWEEN', 
...
C
      WRITE(IFO,3006)IOB
 3006 FORMAT('PITU',A4,2X,'--IN-- TUBE INLET FLUID PRESSURE')
C
      WRITE(IFO,3007)IOB
 3007 FORMAT('WUTU',A4,2X,'--IN-- TUBE OUTPUT MASS FLOW' )
C
      WRITE(IFO,3008)IOB
...
C
    1 WRITE(6,1000)
 1000 FORMAT(//10X,'EXCHANGING TUBE WITH METAL WALL'//)
...
C
      ...
      END
C**************************************************************************
      SUBROUTINE EXCHI2(IFUN,VAR,MX1,IV1,IV2,XYU,DATI,ID1,ID2,IBLOC1,   
...
C
       ...
       ID2=ID2+1
C
C INCREMENTO DI ID2 DOPO MEMORIZZAZIONE DI TAU2H,TAU1H,TAU2P,TAU1P IN EXCHTAU
      ID2 = ID2+4*NCEL                                               
C INCREMENTO DI ID2 DOPO MEMORIZZAZIONE DI GAMMA(NCEL) IN EXCHRES
      ID2 = ID2+NCEL
...
C---- MEMORIZZAZIONE FLAG PER DRENAGGIO CONDENSA
       ...
       ID2=ID2+NCEL
C$$$SPEL
C      WRITE(6,*)' EXCHI2 DATA '
C      DO I=ID1,ID2
C      WRITE(6,*)I,DATI(I)
C      ENDDO
C      WRITE(6,*)'   ******* END OF EXCHI2 DATA'
C$$$SPEL
C                             
...
C                                                                       
      ...
      ENDDO
      K=K+9+NZON(I)
      WRITE(6,3001) I,V,A,DD,C,SPE
   60 CONTINUE                                                          
C                                                                       
C*****
      WRITE(6,'(/)')
      IF(PACO.EQ.1)
     & WRITE(6,*) '   *****  EXCH  -  ZUBER_DIX correlation *****'
      IF(PACO.EQ.2)
     & WRITE(6,*) '   *****  EXCH  -  BANKOFF correlation *****'
      IF(PACO.EQ.3)
     & WRITE(6,*)'  ***  EXCH  -  CHEXAL_LELLOUCHE correlation ***'
      IF(PACO.EQ.4)
     & WRITE(6,*) '   ***  EXCH  -  ZUBER-ROUHANY correlation ***'
      IF(PACO.EQ.0)
     & WRITE(6,*) '   *****  EXCH  -  Omogeneous Model  *****'
C
      RETURN                                                            
 3000 FORMAT(6X,'CELLA',4X,'VOLUME      SECTION     ',      
     $           'INT.DIAM.  HEIGHT DIFF.  EXT.SURFACE'//)                             
 3001 FORMAT(6X,I3,5X,5(E11.4,1X)/)                                     
 3010 FORMAT(6X,3E11.4/)                                                
      END                                                               
C**************************************************************************
      SUBROUTINE EXCHC1(IFUN,AJAC,MX5,IXYU,XYU,IPD,DATI,RNI,IBLOC1,     
     $                  IBLOC2)                                         
C$$$$$     !!!!!!! INIZIO ISTRUZIONI PER SCCS !!!!!!!
       CHARACTER*80 SccsID
       DATA SccsID/'@(#)exch.f	5.16\t 5.16\t 11/3/97'/          
C$$$$$     !!!!!!! FINE ISTRUZIONI PER SCCS !!!!!!!
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z)                              !DBLE
      PARAMETER (NZFMAX=6,NCELMAX=6)
      DIMENSION AJAC(MX5,*),XYU(*),DATI(*),RNI(*)                       
      CHARACTER*4 IBLOC1,IBLOC2,IBLOX1,IBLOX2
      COMMON/EXCH03/CF,PACO,NCEL,NZFTOT,NZON(NCELMAX),NEQ 
      COMMON/EXCH04/H(NCELMAX+1),P(NCELMAX+1),Q(NCELMAX+1),
     $              RO(NCELMAX+1),PN(NCELMAX+1),
     $              HN(NCELMAX+1),HI,HUINV
      COMMON/EXCH05/HM(NCELMAX),TF(NCELMAX),ROM(NCELMAX),
     $            DRDH(NCELMAX),DRDP(NCELMAX),GAMMA(NCELMAX),  
     $            SM(NCELMAX),TITOL(NCELMAX),QTI(NCELMAX),
     $            TPE(NCELMAX),TFL(NZFMAX,NCELMAX),
     $            GE(NZFMAX,NCELMAX)
      COMMON/EXCH06/V(NCELMAX),B(NCELMAX),AREA(NCELMAX),DIAI(NCELMAX),
     $              SUP(NZFMAX,NCELMAX),CKF(NCELMAX),DZG(NCELMAX),
     $              CRIG(NCELMAX),SS(NCELMAX),D(NCELMAX),CM(NCELMAX)             
      COMMON/EXCH07/TAU1H(NCELMAX),TAU2H(NCELMAX),TAU1P(NCELMAX),
...
C      COMMON/INTEG1/TSTOP,TEMPO,DTINT,CDT,ALFADT,NPAS                   !DBLE
      ...
      COMMON/EXCH11/ALF0(NCELMAX+1),SL(NCELMAX+1),C00(NCELMAX+1),
     $             VGJ0(NCELMAX+1),FLU(NCELMAX+1)            
      COMMON/EXCH08/DR(NCELMAX+1),DH(NCELMAX+1),HT(NCELMAX+1)      
      COMMON/EXCH10/IBLOX1,IBLOX2
      LOGICAL KREGIM                                         
C
      NCEL=DATI(IPD)
      DO I=1,NCEL                                                    
...
C                                                                       
   30 CALL EXCHDEC (XYU,IXYU,DATI,IPD,IFUN)                              
C                                                                       
C-----CALCOLO COEFFICIENTI DERIVATE H,P                                 
C                                                                       
      CALL EXCHTAU(IFUN,DATI,IPD)                                                       
C                                                                       
...
C                                                                       
      ...
      IBLOX2=IBLOC2
C
      CALL EXCHRES (IFUN,RNI,DATI,IPD)                                 
C
      RETURN
C                                                                       
 60   CALL EXCHJC(AJAC,MX5,IXYU,XYU,DATI,IPD,RNI,IFUN)                                                                                       
      RETURN                                                            
      END                                                               
C**************************************************************************
      SUBROUTINE EXCHJC(AJAC,MX5,IXYU,XYU,DATI,IPD,RNI,IFUN)            
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z)                              !DBLE
...
C                                                                
      ...
      NVAR=6*NCEL+2*NZFTOT+5                                              
C                                                                       
      CALL EXCHDEC(XYU,IXYU,DATI,IPD,IFUN)                                  
C                                                                       
      CALL EXCHTAU(IFUN,DATI,IPD)                                                       
C
      CALL EXCHRES (IFUN,G1,DATI,IPD)
C                                                                       
      DO J=1,NEQ                                                     
      IF(J.GT.NSTATI) THEN
       G1(J)=-G1(J)                                       
...
C                                                                       
   ...
   15 CONTINUE                                                          
C                                                                       
      CALL EXCHDEC(VAR,1,DATI,IPD,IFUN)                                  
C                                                                       
      CALL EXCHRES (IFUN,G2,DATI,IPD)
C                                                                       
      DO 45 J=1,NEQ                                                     
      IF(J.GT.NSTATI) THEN
       G2(J)=-G2(J)                                       
...
C
      ...
      END           
C**************************************************************************    
      SUBROUTINE EXCHRES (IFUN,RNI,DATI,IPD)                      
C                                                                       
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z)                              !DBLE
      PARAMETER (NZFMAX=6,NCELMAX=6)
     ...
     $            TPE(NCELMAX),TFL(NZFMAX,NCELMAX),
     $            GE(NZFMAX,NCELMAX)
      COMMON/EXCH06/V(NCELMAX),B(NCELMAX),AREA(NCELMAX),
...
C      COMMON/INTEG1/TSTOP,TEMPO,DTINT,CDT,ALFADT,NPAS                   !DBLE
      COMMON/PARPAR/NUL(7),ITERT                                         
      COMMON/EXCH08/DR(NCELMAX+1),DH(NCELMAX+1),HT(NCELMAX+1)
      COMMON/EXCH09/FIQLO(NCELMAX+1),ROLS(NCELMAX+1)
      COMMON/EXCH10/IBLOX1,IBLOX2
      COMMON/REGIME/KREGIM                                              
...
C
      CALL EXCHMET(TM,TMN,TPI,QTE,FM,DATI,IPD,IFUN,DELTAT)
C                                                                       
...
C  CRIG<0  => chiamata alla routine utente EXCHGAMUT
C  CRIG=0  => utilizzo correlazioni semplificate (EXCHGTERT)
...
C
         ...
         CALL EXCHGTERT(PMI,HM(I),SM(I),TF(I),ROM(I),TITOL(I),
     $           TPI(I),DIAI(I),CRIG(I),G,QTI(I),SS(I),GAMMA(I))
         ELSE IF (CRIG(I).LT.0.) THEN ! routine utente
            CALL EXCHGAMUT(PMI,HM(I),SM(I),TF(I),ROM(I),TITOL(I), 
     $           TPI(I),DIAI(I),CRIG(I),G,QTI(I),SS(I),GAMMA(I))        
...
C
      ...
      END
C**************************************************************************
      SUBROUTINE EXCHDEC(XYU,IXYU,DATI,IPD,IFUN)                         
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z)                              !DBLE
      DIMENSION XYU(*),DATI(*)                                          
      PARAMETER (NZFMAX=6,NCELMAX=6)
      COMMON/EXCH03/CF,PACO,NCEL,NZFTOT,NZON(NCELMAX),NEQ                         
...
C---- ESAME I-ESIMO CONFINE (I=1,NCEL)
         ...
         DIAH=DIAI(I)
         ARE=AREA(I)
      CALL EXCHNOM(TIF,ARE,DIAH,IFUN,PACO,Q(I),H(I),P(I),HT(I),
...
C       
     ...
     &     ALF0(NNN)=1./(1.+(1.-TITOL1)/TITOL1*RV/RL)
      ...
      ELSE
         ...
         ARE=AREA(NCEL)
         DIAH=DIAI(NCEL)
       CALL EXCHNOM(TIF1,ARE,DIAH,IFUN,PACO,Q(NNN),H(NNN),P(NNN),
...
C
      ...
      END 
C**************************************************************************
      SUBROUTINE EXCHTAU(IFUN,DATI,IPD)
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z)                              !DBLE
      DIMENSION DATI(*)
      PARAMETER (NZFMAX=6,NCELMAX=6)
      COMMON/EXCH03/CF,PACO,NCEL,NZFTOT,NZON(NCELMAX),NEQ                         
...
C
   ...
   10 CONTINUE                                                          
      ...
      END                                                               
C**************************************************************************
      SUBROUTINE EXCHMET(TM,TMN,TPI,QTE,FM,DATI,IPD,IFUN,DELTAT)
      PARAMETER (NZFMAX=6,NCELMAX=6)
      PARAMETER (PI=3.14159)
...
C
C**************************************************************************
      SUBROUTINE EXCHGTERT(P,H,S,T,RO,TIT,TP,D,CRIG,G,Q,SUP,HTC)
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z)                              !DBLE
C
...
C
      ...
      ENDIF
      IF (TIT.LT.X2) THEN
         DB=EXCHDB(P,S,T,RO,TIT,TP,D,G,X1,ALPHA)
      ELSE IF (TIT.GT.X3) THEN
         HAD=EXCHHAD(P,S,T,RO,TIT,TP,D,G,X4,ALPHA)
      ENDIF
C        
      IF (TIT.LE.X1) THEN
         HTC=DB
...
C
C**************************************************************************
      REAL FUNCTION EXCHDB(P,S,T,RO,TIT,TP,D,G,X1,ALPHA)
C
...
C
      ...
      RE=G*D/MU
      PR=CP*MU/LAM
      EXCHDB=.023*LAM*RE**.8*PR**.4/D
C
      RETURN
      END
C
C**************************************************************************
      REAL FUNCTION EXCHHAD(P,S,T,RO,TIT,TP,D,G,X4,ALPHA)
C
      REAL MU,LAM,MUMIN,LAMMIN
      DATA MUMIN/9.E-06/,LAMMIN/18.E-03/,CPMIN/1000./
C
...
C
      ...
      RE=G*D/MU
      PR=CP*MU/LAM
      EXCHHAD=.008348*LAM*RE**.8774*PR**.6112/D
C
      RETURN
      END
C        
C**************************************************************************
      SUBROUTINE EXCHD1 (BLOCCO,NEQUAZ,NSTATI,NUSCIT,NINGRE,SYMVAR,XYU,
...
C--- Equazione di conservazione dell'energia del metallo
      ...
      END
C**************************************************************************
      REAL FUNCTION EXCHISTA09(IFUN)
C      DOUBLE PRECISION FUNCTION EXCHISTA09(IFUN)                            !DBLE
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z)                              !DBLE
      COMMON/REGIME/KREGIM                                              
...
C                                                                       
      EXCHISTA09=0                                                          
      IF(KREGIM) EXCHISTA09=1                                               
      IF((KSTP.EQ.1).AND.(ITERT.EQ.0)) EXCHISTA09=1                      
C
      RETURN                                                            
      END                                                               
C**************************************************************************
      SUBROUTINE EXCHTA(ISTA)                                           
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z)                              !DBLE
      PARAMETER (NZFMAX=6,NCELMAX=6)
      COMMON/EXCH03/CF,PACO,NCEL,NZFTOT,NZON(NCELMAX),NEQ                         
      COMMON/EXCH04/H(NCELMAX+1),P(NCELMAX+1),Q(NCELMAX+1),
...
C      WRITE(6,1850)AMT                                           
 ...
 1850 FORMAT(/,1X,' MASSA DEL TRONCONE COMPLETO:  ',F12.6)                     
      RETURN 
      END                                                               
      FUNCTION EXCHG9(IFUN)                                             !SNGL
C      DOUBLE PRECISION FUNCTION EXCHG9(IFUN)                            !DBLE
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z)                              !DBLE
      COMMON/REGIME/KREGIM                                              
      COMMON/PARPAR/NUL(6),KSTP,ITERT                                   
      LOGICAL KREGIM                                                    
C                                                                       
C-----EXCHG9=0 NON CALCOLA IL COEFFICIENTE DI SCAMBIO                   
C-----EXCHG9=1 CALCOLA IL COEFFICIENTE DI SCAMBIO                       
C                                                                       
    1 EXCHG9=1                                                          
      IF(KREGIM)RETURN                                                  
      IF(ITERT.NE.0)EXCHG9=0                                            
      IF(IFUN.NE.2)EXCHG9=0                                             
      RETURN                                                            
      END                                                               
C**************************************************************************
       SUBROUTINE EXCHNOM(TIF,AREA,DIAH,IFUN,PACO,QQI,HHI,PPI,HTI,
     &                   RM,DRD,DHD,ALFN,SLN,C0,VGJ,FL,I,FIQLOJ,RL)       
C****************************************************************************
C    Alla subroutine EXCHNOM il modulo si rivolge sempre tranne che per 
...
C            chiama la EXCHUGJALF che calcola UGJ e ALF;
C          - tornando alla EXCHNOM e' ora possibile calcolare i valori
...
C
C>>>>  Versione di "EXCHNOM" modificata da L. POLLACHINI il 05/12/1991  <<<<
...
C
        ...
        SIG=EXCHTENSUP(DTK) 
C 
        IF (PACO.EQ.1.)  CALL EXCHZD(RL,RV,XV,FL,SIG,AREA,
     $                               UGJ,ALFN,C0,VGJ,XP)
C
        IF (PACO.EQ.2.)  CALL EXCHBKO(PPI,XV,FL,UGJ,ALFN,C0,VGJ)
C
        IF (PACO.EQ.3.)  CALL EXCHCHELEL(PPI,XV,FL,ALF,SIG,RV,RL,TIF,
     $                               DIAH,UGJ,ALFN,C0,VGJ) 
C
        IF (PACO.EQ.4.)  CALL EXCHZUBROU(RL,RV,FL,XV,SIG,G,DIAH,
     $                               UGJ,ALFN,C0,VGJ,XP)
C        IF (IFUN.EQ.2) THEN         
...
C        ENDIF
 ...
 950    RETURN      
        END
C
        SUBROUTINE EXCHZD(RL,RV,XV,FL,SIG,AREA,UGJ,ALFN,C0,VGJ,XP)
C
...
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z)                              !DBLE
        ...
        CALL EXCHUGJALF(C0,FL,VGJ,XV,UGJ,ALF) 
        ALFN=ALF
        RETURN
        END
C
        SUBROUTINE EXCHBKO(P,XV,FL,UGJ,ALFN,C0,VGJ)
C
...
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z)                              !DBLE
        ...
        END 
C
        SUBROUTINE EXCHCHELEL(PPI,XV,FL,ALF,SIG,RV,RL,TIF,
     $             DIAH,UGJ,ALFN,C0,VGJ)
C
...
C    CALCOLO VISCOSITA'
C
        CALL EXCHVISCO(TIF,UL,UV) 
C
  50    CALL EXCHRECHEX(RL,RV,VG,VL,DIAH,UL,UV,REG,REL,RE)
        CALL EXCHC0CHEX(RE,RA,PR,ALF,B1,C0)
...
C
        ...
        END
C
        SUBROUTINE EXCHUGJALF(C0,FL,VGJ,XV,UGJ,ALF)
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z)                              !DBLE
        UGJ=(C0-1.)*FL+VGJ
        ALF= XV/(C0+VGJ/FL)
        RETURN
        END
C
        SUBROUTINE EXCHC0CHEX (RE,RA,P,ALF,B1,C0)
C
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z)                              !DBLE
        REAL K0,L                                                       !SNGL
...
C
        ...
        END
C
        SUBROUTINE EXCHRECHEX(RL,RV,VG,VL,DIAH,UL,UV,REG,REL,RE)
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z)                              !DBLE
        REL= VL*RL*DIAH/UL 
        REG= VG*RV*DIAH/UV
...
C
        ...
        END
C 
        REAL FUNCTION EXCHTENSUP(T)                                         !SNGL
C        DOUBLE PRECISION FUNCTION EXCHTENSUP(T)                             !DBLE
C
C                   calcolo tensione superficiale
C 
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z)                              !DBLE
        EXCHTENSUP=.001*((.1161*T**2.)/(1.+.83*T)+(1.121E-03)*T**2.
     $           -(5.752E-06)*T**3.+(1.286E-08)*T**4.
     $           -(1.149E-11)*T**5.) 
        RETURN
        END
        SUBROUTINE EXCHZUBROU(RL,RV,FL,XV,SIG,G,DE,UGJ,ALFN,C0,VGJ,XP)
C
...
C
C    SUBROUTINE EXCHGAMUT(P,H,S,TF,RO,TITOL,TP,DIA,CRIG,G,Q,SUP,HTC)
C
C  Calcolo di HTC = coefficiente di scambio termico in [W/m**2/K]   
C    
C__________ SE VUOI CHE LA SUBROUTINE EXCHGAMUT SIA CHIAMATA DA EXCH
C           RICORDATI DI METTERE NEI DATI DEL MODULO SU FILE F14
C
C                   CRIG < 0.
C
C  La subroutine fornisce al modulo EXCH il coefficiente di scambio 
