...
C release 5.2
C data 4/12/95
C reserver @(#)cpom.f	5.2
C**********************************************************************
C
      SUBROUTINE CPOMI3(IFO,IOB,DEBL)
...
C
C
      SUBROUTINE CPOMI2(IFUN,VAR,MX1,IV1,IV2,XYU,DATI,ID1,ID2,
...
C
   70 RETURN
      END
C
      SUBROUTINE CPOMC1(IFUN,AJAC,MX5,IXYU,XYU,IPD,DATI,RNI,
     $                  IBLOC1,IBLOC2)
C
C------POMPA CENTRIFUGA
C
...
C
   20 CALL PMPJC(AJAC,MX5,IXYU,XYU,DATI,IPD,RNI,IFUN,
     $           IBLOC1,IBLOC2)
      RETURN
      END
C
      SUBROUTINE PMPDEC(XYU,IXYU,ECIN,OM,HV,PM,PV,Q,HM,WM,
...
C
  ...
  17  IF(IWP.EQ.0)GO TO 20
   ...
   20 CONTINUE
      ...
      END
C
      SUBROUTINE PMPDR(PMN,PVN,HMN,HVN,QN,OMN,ECINN,ETANOM,
...
C------CALCOLO RESIDUI
C
      IF(TYP.LE.0.) CALL PMPDPC(GCAR,QNOM,OMNOM,DPNOM,CNOM,
     $                          Q1PV,OM,DP,CRES)
      IF(TYP.GT.0.) CALL DPU(TYP,QNOM,OMNOM,DPNOM,CNOM,ETANOM,
     $                       Q1PV,RO,OM,DP,CRES)
      WR=CRES*OM*NPO
      GO TO (16,17),IHI
C
 16   IF(KREGIM)GO TO 1000
      CALL CPOTER(DATI,IPD,Q,HM,HV,WR,PM,PV,HVT)
      RNI(1)=(HV-HVT)/H0
      GO TO 1010
C
...
C
      ...
      END
C
      SUBROUTINE PMPJC(AJAC,MX5,IXYU,XYU,DATI,IPD,RNI,IFUN,
     $                 IBL1,IBL2)
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z)                              !DBLE
      COMMON/REGIME/KREGIM
      LOGICAL KREGIM
...
C
      CALL PMPDEC(XYU,IXYU,ECIN,OM,HV,PM,PV,Q,HM,WM,
     $            DATI,IPD,IHI,IWP,TYP,HIN,NPO,IBL1,IBL2,
     $            QNOM,OMNOM,DPNOM,CNOM,GCAR,ECINOM,ETANOM,PERD,
     $            VALV)
      CALL PMPDR(PM,PV,HM,HV,Q,OM,ECIN,ETANOM,
...
C
   ...
   15 CONTINUE
      CALL PMPDEC(VAR,1,ECIN,OM,HV,PM,PV,Q,HM,WM,
     $            DATI,IPD,IHI,IWP,TYP,HIN,NPO,IBL1,IBL2,
     $            QNOM,OMNOM,DPNOM,CNOM,GCAR,ECINOM,ETANOM,PERD,
     $            VALV)
      CALL PMPDR(PM,PV,HM,HV,Q,OM,ECIN,ETANOM,
...
C
   ...
   31 CONTINUE
      ...
      END
C
      SUBROUTINE PMPDPC(GCAR,QNOM,OMNOM,DPNOM,CNOM,Q,OM,DP,CRES)
C
C ----------------- PARAMETRI FISICI --------------------
...
C-------RMOLT=ROM**2.+RQ**2.---------A.ROSSI, 20/2/89
      ...
      END
C
      SUBROUTINE CPOTER(DATI,IPD,Q,HM,HV,POT,PM,PV,HVT)
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z)                              !DBLE
      COMMON/PARPAR/NUL(7),ITERT
...
C
C
      SUBROUTINE CPOMD1(BLOCCO,NEQUAZ,NSTATI,NUSCIT,NINGRE,SYMVAR,
