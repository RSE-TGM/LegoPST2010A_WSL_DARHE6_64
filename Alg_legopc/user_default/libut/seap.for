      SUBROUTINE SEAPI3(IFO,IOB,DEBL)
C
      CHARACTER*4 IOB, MOD, IBLOC*8, DEBL*80
      DATA MOD/'SEAP'/
C
      CALL SEAPI4 (IOB, MOD, IBLOC, NE, NU)
      WRITE(IFO,2999)IBLOC,IOB,MOD,DEBL
      WRITE(IFO,3000)
     $  'TMET',IOB,'US','TEMPERATURE OF THE RESERVOIR METAL WALL',
     $  'PSER',IOB,'UA','AIR/GAS PRESSURE IN THE RESERVOIR',
     $  'TSER',IOB,'UA','AIR/GAS TEMPERATURE IN THE RESERVOIR'
      WRITE(IFO,3001)
...
C
C
      SUBROUTINE SEAPI4 (IOB, MOD, IBLOC, NE, NU)
...
C
C
      SUBROUTINE SEAPI2(IFUN,VAR,MX1,IV1,IV2,XYU,DATI,ID1,ID2,
...
C
    ...
    1   FORMAT (/5X,'******** MODULE SEAP - BLOCK ', 2A4, ' ********',
...
C
C
      SUBROUTINE SEAPC1(IFUN,AJAC,MX5,IXYU,XYU,IPD,DATI,RNI,IBL1,IBL2)
C
      PARAMETER (MAX = 40)
...
C---calcolo residui
C
  200 CALL SEAP(IFUN,IXYU,XYU,IPD,DATI,RNI)
      RETURN
C
C---calcolo jacobiano
C
  300 NSTATI = 1
     ...
     $           NSTATI,NUSCIT,NINGRE,EPS,EPSLIM,SEAP)
      RETURN
C
      END
C
C
C
      SUBROUTINE SEAP(IFUN,IXYU,XYU,IPD,DATI,RNI)
C
...
C
      IF (KREGIM) THEN
        RNI(1) = ((XCSI * XAINT * TSER + XCSE * XAEST * TEST) /
     $            (XCSI * XAINT + XCSE * XAEST) - TMET) / T0
      ELSE
        RNI(1) = (QINT - QEST) / (AMMAS * CMET * T0)
      ENDIF
C
      RNI(2) = (SUMI + SUMU) / W0
C
      IF (SUMI .LT. 5.) THEN
        IF (SUMI .LT. 0.01) THEN
          HUMHI = TI(1) * CP
        ELSE
          HUMHI = SUMHI / SUMI
        ENDIF
...
C
C
      SUBROUTINE SEAPD1 (BLOCCO,NEQUAZ,NSTATI,NUSCIT,NINGRE,SYMVAR,XYU,
