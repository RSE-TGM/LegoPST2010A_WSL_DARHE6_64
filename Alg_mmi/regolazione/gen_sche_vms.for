C******************************************************************************
C modulo gen_sche_vms.for
C tipo 
C release 5.1
C data 11/13/95
C reserver @(#)gen_sche_vms.for	5.1
C******************************************************************************
      PROGRAM GEN_SCHE
C
C
C
C               PROGRAMMA   GEN_SCHE
C
...
C                           output del programma.
      COMMON/NOM_REG/SIMB_REG,DESC_REG
      CHARACTER*4 SIMB_REG
      CHARACTER*200 DESC_REG
      CHARACTER*4 SIMB_PAG
      CHARACTER*4 SIMB_FILE
      INTEGER*4 LUN_SIMB_PAG

      CHARACTER*200 PATH_U_CC
      CHARACTER*200 PATH
      INTEGER*4 LUN_SIMB_PATHU
      INTEGER*4 LUN_SIMB_PATH
C
C____ FILES DI LAVORO DI GEN_SCHEMA
      CHARACTER*200 FILETOP
C
C____ FILES DI USCITA DI GEN_SCHEMA
      CHARACTER*200 F01_DAT
      CHARACTER*200 NOM_FOR,NOM_DAT

      CHARACTER*200 LINE



C     Lettura dai nomi logici SCHEMA_REG, PATH_U_CC del
C     nome della  pagina di regolazione  trattata e del
C     path in cui vengono riposti i files prodotti.

      LUN_SIMB_PAG = 100
      CALL TRA_NOM_LOG_FOR(SIMB_PAG,LUN_SIMB_PAG,'SCHEMA_REG',
     &                     'LNM$PROCESS_TABLE')

      LUN_SIMB_PATHU = 200
      CALL TRA_NOM_LOG_FOR(LINE,LUN_SIMB_PATHU,'PATH_U_CC',
     &                     'LNM$PROCESS_TABLE')

      write(*,*) '--------------------'
      write(*,*) SIMB_PAG,LUN_SIMB_PAG
      write(*,*) LINE,LUN_SIMB_PATHU
      write(*,*) '--------------------'

      CALL CONV_LOWER(LINE,80,PATH_U_CC)
      CALL CONV_LOWER(SIMB_PAG,4,SIMB_FILE)

C  PATH viene posto uguale a PATH_U_CC in quanto non solo il file 
C  SIMB_REG.f viene collocato nella directory PATH_U_CC ma  anche 
C  gli altri due file prodotti : SIMB_REG_01.dat SIMB_REG.dat 
      PATH = PATH_U_CC
      LUN_SIMB_PATH = LUN_SIMB_PATHU
...
556   CONTINUE
        ...
        FILETOP = PATH_U_CC(1:LUN_SIMB_PATHU)//
     &               SIMB_PAG(1:LUN_SIMB_PAG)//'.top'

        F01_DAT  = PATH_U_CC(1:LUN_SIMB_PATHU)//
     &               SIMB_FILE(1:LUN_SIMB_PAG)//'_01.dat'

        NOM_DAT  = PATH_U_CC(1:LUN_SIMB_PATHU)//
     &               SIMB_FILE(1:LUN_SIMB_PAG)//'.dat'

        NOM_FOR  = PATH(1:LUN_SIMB_PATH)//
     &               SIMB_FILE(1:LUN_SIMB_PAG)//'.for'


C       write(*,*) SIMB_PAG,LUN_SIMB_PAG
C       write(*,*) SIMB_FILE,LUN_SIMB_PAG
C       write(*,*) SIMB_REG,LUN_SIMB_PAG
C       write(*,*) PATH_U_CC,LUN_SIMB_PATHU
...
C      Apertura del file simb_pag.top
       OPEN(UNIT=1,FILE=FILETOP,STATUS='OLD',
     &      RECORDTYPE='STREAM_LF')

C      Apertura del file simb_file_01.dat
       OPEN(UNIT=2,FILE=F01_DAT,STATUS='NEW',
     &      RECORDTYPE='VARIABLE',CARRIAGECONTROL='LIST')
       REWIND(2)
C
C
C
...
C
C       Apertura del file simb_file.for
	OPEN(UNIT=12,FILE=NOM_FOR,STATUS='NEW',
     &       RECORDTYPE='VARIABLE',CARRIAGECONTROL='LIST')
        REWIND(12)
C
	DO I=1,LI
...
C
C       Apertura del file simb_file.dar
	OPEN(UNIT=12,FILE=NOM_DAT,STATUS='NEW',
     &       RECORDTYPE='VARIABLE',CARRIAGECONTROL='LIST')
...
C
C_____ LETTURA DELLA DESCRIZIONE DELLO SCHEMA
C
      IF(LINE(1:4).EQ.SIMB_REG)DESC_REG=LINE(11:60)
C
      ICLIN=ICLIN+1
C
...
C
	...
	FILREG(KLIN+1)=SIMB_REG//'      BLOCCO ('//SIMB_REG//
     &  ') - '//DESC_REG
	FILREG(KLIN+2)='****'
	FILREG(KLIN+3)=SIMB_REG//SIMB_REG//'  BL.-'//SIMB_REG//
