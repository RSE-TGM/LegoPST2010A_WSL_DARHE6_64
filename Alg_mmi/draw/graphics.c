...
//static PAGINA* pag_in_move = NULL; // Aggiungi questa per avere il contesto della pagina

// // Variabili statiche per gestire lo stato del drag
// static PAGINA*    pag_in_move = NULL;      // Puntatore alla pagina dove avviene lo spostamento
// static Widget*    selected_widgets = NULL; // Lista dei widget selezionati da spostare
// static Cardinal   num_selected = 0;        // Numero di widget selezionati
// static Boolean    drag_in_progress = False;  // Flag per indicare se uno spostamento è in corso
static Position   start_drag_x, start_drag_y; // Coordinate iniziali del mouse (root)

static PAGINA*    pag_in_move = NULL;
static Widget*    selected_widgets = NULL; // NOTA: 'selected' era un nome troppo generico
static Cardinal   num_selected = 0;
static Boolean    is_dragging = False;
// Coordinate iniziali del mouse relative alla finestra del widget che ha ricevuto il BtnDown
static int        drag_start_x, drag_start_y; 
// Array delle posizioni X e Y iniziali dei widget
static Position*  original_widgets_x = NULL;
static Position*  original_widgets_y = NULL;

// Funzione helper per liberare la memoria usata durante il drag
static void CleanupDragState() {
    if (selected_widgets) { libera_memoria((char*)selected_widgets); selected_widgets = NULL; }
    if (original_widgets_x) { libera_memoria((char*)original_widgets_x); original_widgets_x = NULL; }
    if (original_widgets_y) { libera_memoria((char*)original_widgets_y); original_widgets_y = NULL; }
    num_selected = 0;
    pag_in_move = NULL;
    is_dragging = False;
}

// ACTION: Chiamata al <Btn1Down>
void StartOrSelect(Widget w, XEvent *event, String *params, Cardinal *num_params)
{
    Widget drawing_area = w;
	printf("StartOrSelect called on widget %s\n", XtName(w));
    while (XtParent(drawing_area) && !XtIsSubclass(drawing_area, xmDrawingAreaWidgetClass)) {
        drawing_area = XtParent(drawing_area);
    }
    
    pag_in_move = GetPaginaFromWidget(drawing_area);
    if (!pag_in_move) return;

    Boolean clicked_on_selected = iswidsel(w);

    if (!(event->xbutton.state & ShiftMask) && !clicked_on_selected) {
        pag_deselect_all(drawing_area);
    }

    if (event->xbutton.state & ShiftMask) {
        set_something(w, XlNselected, (void*)!clicked_on_selected);
    } else {
        set_something(w, XlNselected, (void*)True);
    }

    if (get_selected(pag_in_move, &selected_widgets, &num_selected)) {
        is_dragging = True;
        drag_start_x = event->xbutton.x;
        drag_start_y = event->xbutton.y;

        original_widgets_x = (Position*)alloca_memoria(num_selected, sizeof(Position));
        original_widgets_y = (Position*)alloca_memoria(num_selected, sizeof(Position));

        for (int i = 0; i < num_selected; i++) {
            XtVaGetValues(selected_widgets[i], XmNx, &original_widgets_x[i], XmNy, &original_widgets_y[i], NULL);
        }
    }
}

// ACTION: Chiamata al <Btn1Motion>
void MoveIt(Widget w, XEvent *event, String *params, Cardinal *num_params)
{
	printf("MoveIt called with %d selected widgets is_dragging=%d\n", num_selected, is_dragging);
    if (!is_dragging || num_selected == 0) return;

    // Calcola il delta relativo al movimento del mouse all'interno del widget `w`
    int dx = event->xmotion.x - drag_start_x;
    int dy = event->xmotion.y - drag_start_y;

    for (int i = 0; i < num_selected; i++) {
        XtVaSetValues(selected_widgets[i],
                      XmNx, original_widgets_x[i] + dx,
                      XmNy, original_widgets_y[i] + dy,
                      NULL);
    }
}

// ACTION: Chiamata al <Btn1Up>
void EndMove(Widget w, XEvent *event, String *params, Cardinal *num_params)
{
	printf("EndMove called with %d pag_in_move widgets is_dragging=%d\n", pag_in_move, is_dragging);
    if (!is_dragging || !pag_in_move) return;

    // Ricalcola il delta finale per la massima precisione
    int dx = event->xbutton.x - drag_start_x;
    int dy = event->xbutton.y - drag_start_y;

    for (int i = 0; i < num_selected; i++) {
        Position final_x = original_widgets_x[i] + dx;
        Position final_y = original_widgets_y[i] + dy;
        
        assegna_xyfisiche(pag_in_move->db, selected_widgets[i], final_x, final_y);
    }
    
    if (!strcmp(pag_in_move->geom.tipo, TIPO_REGOLAZIONE)) {
        XRectangle final_positions[num_selected];
        for(int i = 0; i < num_selected; i++) {
             get_geometry(selected_widgets[i], &final_positions[i].x, &final_positions[i].y, &final_positions[i].width, &final_positions[i].height);
        }
        select_connections(num_selected, selected_widgets, final_positions, RECTMOVE);
    }
	...
	static XtActionsRec actions[] =
		{
			...
			{"end_draw_conn", (void*)EndDrawConn},
...
// void StartPick(Widget wid, XEvent *ev, String *params, Cardinal *num_params)
// {
//     Widget drawing_area = wid;
//     printf("StartPick called drag_in_progress=%d\n", drag_in_progress);
//     // Se stiamo già facendo un drag, ignora questo evento per sicurezza
//     if (drag_in_progress) return;

//     // Trova la drawingArea e la pagina associate
//     while (XtParent(drawing_area) && !XtIsSubclass(drawing_area, xmDrawingAreaWidgetClass)) {
//         drawing_area = XtParent(drawing_area);
//     }
//     pag_in_move = GetPaginaFromWidget(drawing_area);
...
//     // Controlla se il click è su un widget già selezionato
//     Boolean clicked_on_selected = iswidsel(wid);

//     // Se il click NON è su un widget già selezionato, deseleziona tutto
//     if (!clicked_on_selected) {
//         pag_deselect_all(drawing_area);
//         DeselectAllDraget(drawing_area, True);
//         // E seleziona solo il widget corrente
//         set_something_val(wid, XlNselected, (XtArgVal)True);
//     }

//     // Ora recupera la lista (nuova o vecchia) dei widget selezionati
//     if (get_selected(pag_in_move, &selected_widgets, &num_selected)) {
//         drag_in_progress = True;
//         start_drag_x = ev->xbutton.x_root;
//         start_drag_y = ev->xbutton.y_root;
        
//         // Attiva la translation table per lo spostamento
//         XtOverrideTranslations(drawing_area, XtParseTranslationTable(pag_move_translations));
//     } else {
//         // Non siamo riusciti a ottenere una selezione, pulisci tutto
//         pag_in_move = NULL;
//     }
// }

// void MovePick(Widget wid, XEvent *ev, String *params, Cardinal *num_params)
// {
// 	printf("MovePick called drag_in_progress=%d\n", drag_in_progress);
//     if (!drag_in_progress || num_selected == 0) return;

//     int dx = ev->xmotion.x_root - start_drag_x;
//     int dy = ev->xmotion.y_root - start_drag_y;

//     for (int i = 0; i < num_selected; i++) {
//         Position old_x, old_y;
//         Dimension temp_w, temp_h;
//         get_geometry(selected_widgets[i], &old_x, &old_y, &temp_w, &temp_h);
        
//         // Applica lo spostamento visivo
//         XtVaSetValues(selected_widgets[i], XmNx, old_x + dx, XmNy, old_y + dy, NULL);
//     }
    
//     start_drag_x = ev->xmotion.x_root;
//     start_drag_y = ev->xmotion.y_root;
// }

// //void EndPick(Widget wid, XEvent *ev, String *params, Cardinal *num_params)
// void EndPick(Widget wid, XEvent *ev)
// {
// 	printf("EndPick called drag_in_progress=%d\n", drag_in_progress);
//     if (!drag_in_progress || !pag_in_move) return;

//     for (int i = 0; i < num_selected; i++) {
//         Position final_x, final_y;
//         Dimension temp_w, temp_h;
//         get_geometry(selected_widgets[i], &final_x, &final_y, &temp_w, &temp_h);
//         assegna_xyfisiche(pag_in_move->db, selected_widgets[i], final_x, final_y);
//     }
    
//     // Ripristina le translation di default
//     char default_trans_str[200];
//     sprintf(default_trans_str, "#override <Btn1Down>: start_pick(%p)", pag_in_move);
//     XtOverrideTranslations(pag_in_move->drawing, XtParseTranslationTable(default_trans_str));

//     // Pulisci lo stato e libera la memoria
//     drag_in_progress = False;
//     pag_in_move = NULL;
//     if (selected_widgets) {
//         libera_memoria((char*)selected_widgets);
...
Widget draw_wid;
{
	...
	unsigned long valuemask = GCForeground | GCBackground | GCLineWidth | GCLineStyle | GCSubwindowMode;
	XGCValues values;
	...
	Draget draw_wid;
{
	...
	*/
	...
	{
		DrawDeleteAll((Widget) draw_wid, NULL);
/*
 Fine codice CAPPE
*/
#ifdef MAURIC
