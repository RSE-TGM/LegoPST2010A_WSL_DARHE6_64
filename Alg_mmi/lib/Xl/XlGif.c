...
int  ReadCode();
int  AddToPixel(byte);
int  ColorDicking(Widget, char *);


/*****************************/
LoadGIF(Widget w, char *fname, XImage **theImage)
/*****************************/
{
    int            filesize;
...
/* Parte di reinizializzazione inserita per la lettura di un successivo
   GIF file */

    BitOffset = 0;		/* Bit Offset of next code */
    XC = 0;
    YC = 0;		/* Output X and Y coords of current pixel */
    Pass = 0;			/* Used by output routine if interlaced pic */
    OutCount = 0;		/* Decompressor output 'stack count' */
    ...
    if (!fp) XlWarning("XlGif","LoadGIF","file not found");
	...
	XlWarning("XlGif","LoadGIF - RawGIF","not enough memory to read gif file");

    if (!(Raster = (byte *) malloc(filesize)))
	XlWarning("XlGif","LoadGIF - Raster","not enough memory to read gif file");

    if (fread(ptr, filesize, 1, fp) != 1)
	XlWarning("XlGif","LoadGIF","GIF data read failed");

    if (strncmp((char *)ptr, (char *)id, 6))
	XlWarning("XlGif","LoadGIF","not a GIF file");
...
/* Get variables from the GIF screen descriptor */
	...
	XlWarning("XlGif","LoadGIF","Corrupt GIF file (bad screen descriptor)");
...
/* Read in global colormap. */
	...
	if (Verbose)
		...
		fname, Width,Height,BitsPerPixel, numcols);
...
if (Verbose) {
	    ...
	    for (i=0; i<numcols; i++) fprintf(stderr, "XlGif===> ch=%x IMAGESEP=%x image separator R[%d]=%x,G[%d]=%x,Blue[%d]=%x\n", ch, IMAGESEP,i,Red[i],i,Green[i],i,Blue[i] );
}
//    if (NEXTBYTE != IMAGESEP)
    if (ch != IMAGESEP)
	XlWarning("XlGif","LoadGIF","corrupt GIF file (no image separator)");
 ...
 */
	...
	if ((Raster - ptr1) > filesize)
	    XlWarning("XlGif","LoadGIF","corrupt GIF file (unblock)");
    } while(ch1);
...
/* Allocate the X Image */
    ...
    if (!Image) XlWarning("XlGif","LoadGIF","not enough memory for XImage");
    
 //   (*theImage) = XCreateImage(theDisp,theVisual,8,ZPixmap,0,(char *)Image,
//                         Width,Height,8,Width);

   (*theImage) = XCreateImage(theDisp,theVisual,24,ZPixmap,0,(char *)Image,
                         Width,Height,8,Width);
    if (!(*theImage)) XlWarning("XlGif","LoadGIF","unable to create XImage");
...
/*************************/
int ColorDicking(Widget w,char *fname)
{
    ...
    Display *theDisp=XtDisplay(w);
    int theScreen=XScreenNumberOfScreen(XtScreen(w)); 
    Colormap theCmap=DefaultColormap(theDisp, theScreen);
    int nostrip=0; /* settato come per default */
        ...
        if (j) {		/* failed to pull it off */
            ...
            for (i=0; i<numcols; i++)
                ...
                if (used[i] && cols[i]==0xffff) {  /* an unallocated pixel */
                    ...
                    if (close<0) XlWarning("XlGif","LoadGIF","simply can't do it.  Sorry.");
...
/**
 Funzioni di livello superiore per il caricamento del .GIF
 in un Pixmap di dimensioni opportune.
**/



Boolean XlCreatePixmapFromGIF(w, nome_file_GIF, thePixmap)
Widget w; /* Widget di riferimento per display, screen ecc... */
char *nome_file_GIF; /* nome file GIF completo di estensione */
Pixmap *thePixmap; /* pixmap allocato internamente alla funzione
                      (deve essere passato come puntatore) */
{
        FILE *fp;
        char *disegni_dir;
        ...
        XImage *theImage;
        /*
         Verifica dell'esistenza del file .GIF richiesto
        */
/*
        verifico se il file si trova nella directory corrente
*/
        if((fp=fopen(nome_file_GIF,"r"))!=NULL) /* OK. file trovato */
                {
                fclose(fp);
                strcpy(path_bitmap,nome_file_GIF);
                }
/*
        Altrimenti lo cerco nella directory puntata da LEGOMMI_ICO
*/
        else
                {
                ...
                strcat(path_bitmap,"/");
                strcat(path_bitmap,nome_file_GIF);
        ...
        *thePixmap=XCreatePixmap (XtDisplay(w),
...
/*        *thePixmap=XCreatePixmap (XtDisplay(w),
                           ...
                           8 );
*/
printf("\n XlCreatePixmapFromGIF: fine LoadGIF %d x %d depth=%d",theImage->width, theImage->height,
(unsigned int)DefaultDepth(XtDisplay(w),XScreenNumberOfScreen(XtScreen(w))));
fflush(stdout);

        XPutImage(XtDisplay(w),*thePixmap,
                        theGC,theImage,0,0,0,0,
                        theImage->width,theImage->height);
printf("\n XlCreatePixmapFromGIF: fine XPutImage");
