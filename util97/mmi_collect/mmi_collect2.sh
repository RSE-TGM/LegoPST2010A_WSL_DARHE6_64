#!/bin/ksh
#@(#)mmi_collect2	Rifacimento mmi_collect				1997-07-09
...
Uso:
	mmi_collect2  [ -dhvx -g glob-dir -w win-dir ] xxxxx
...
Version()
{
	echo	"mmi_collect2  -  Versione 1.1  1997-07-02"
	exit	$1
}					# Version()
...
scr=mmi_collect2.scr				# Output

egrep1=mmi_collect2.egrep			# Utilizzato dalla egrep
pagmod=`which pagmod`					# Eseguibile
page_num=0					# Risorsa: *pag_num   / Context.ctx
page_list=""					# Risorsa: *page_list / Context.ctx
pwd=`pwd`
s02=$pwd/S02_OSF1
t2=mmi_collect2.t2				# F. temporaneo (anti: arg too long)
...
efn=mmi_collect2.err			# Log degli errori

date '+mmi_collect2 - Inizio %y-%m-%d %T' > $efn
...
#...	sdn=......
ls -1 $wdn/O_*.rtf 2>> $efn | while read lfn; do
	D "Link_rtf $lfn ....."
	Link_rtf $lfn
done

ls -1 $wdn/F_*.rtf 2>> $efn | while read lfn; do
	D "Link_rtf $lfn ....."
	Link_rtf $lfn
done

ls -1 $wdn/M_*.rtf 2>> $efn | while read lfn; do
	D "Link_rtf $lfn ....."
	Link_rtf $lfn
done
...
do
	...
	X "egrep -f mmi_collect2.egrep $rfn | sed -e \"s/./*$tsk./\" >> $ctx"
	egrep -f mmi_collect2.egrep $rfn | sed -e "s/./*$tsk./" >> $ctx
done
...
date '+mmi_collect2 - Fine   %y-%m-%d %T' >> $efn
