...
char *car;
/*
inserire controllo per verificare che la variabile non sia gi� 
stata inserita
*/
...
/*
Se il task � di regolazione cambia la var sulla 1a colonna
*/
...
if(nome_task!=NULL)
	...
	if(strlen(stringhe[0])>6)
		{
		printf("Variabili gi� convertite\n");
		exit(1);		
		}
...
/*
Se il task � di regolazione cambia nome variabile
*/	
...
/*
controllo per verificare se le var sono gi� state modificate
*/		
		if(strlen(stringhe[2])>6)/*modifica*/
			{
			printf("Variabile gi� modificata\n");
			exit(1);
			}
...
/*
Se leggi *** allora la prossima linea conterr� un task
*/	
	if(*linea=='*')
		{
		next_task=1;
