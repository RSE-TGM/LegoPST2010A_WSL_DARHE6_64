#!/bin/ksh
#
#  Script:			mmi_collect4.sh
#  Instance:		1
#  %version:		3 %
#  Description:		
#  %created_by:		lomgr %
#  %date_created:	Fri Sep 10 12:51:27 2004 %

#@(#)mmi_collect4	Rifacimento mmi_collect				1997-07-09
...
Uso:
	mmi_collect4  [ -dhvx -g glob-dir -w win-dir ] xxxxx
...
Version()
{
	echo	"mmi_collect4  -  Versione 1.1  1997-07-02"
	exit	$1
}					# Version()
...
#	Estrae posizione e offset della task da variabili.edf
        ...
        export f4=0
        export f5=0
	grep "^NOME MODELLO.* $task " $vfn | while read f1 f2 f3 f4 f5
        do
           echo $f1 $f2 $f3 $f4 $f5 $task $vfn
...
#+	echo "$ss `ls -g $bkg` $rr"
	if [ -f $bkg ]; then
		D "Riloca $rfn ...."
		Riloca $rfn
        echo $page_num >  VascoRossi.valore_di_pagenum
        echo $page_list > VascoRossi.valore_di_pagelist
	else
		X "rm -f $rfn $bkg"
		rm -f $rfn $bkg
#+		sleep 2
...
#//	echo "+++ chgcompiled posizione offset < file > savepwd/GLBPAGES/file"
	...
	rdn=`dirname $rfn`
        firstchar_rdn=`echo $rdn | cut -c 1`
        if [ "`echo $rdn | cut -c 1`" = "." ]
        then
           rdn=../$rdn
        fi 
...
Default	MMI_HOSTNAME		""			# ..........
Default	MMI_HOSTNAME_SCADA	""			# ..........
scr=mmi_collect4.scr				# Output

direxe=~/legocad/libut_bin                      # Direttorio eseguibili

egrep1=mmi_collect4.egrep			# Utilizzato dalla egrep
pagmod=`which pagmod`				# Eseguibile
typeset page_num=0					# Risorsa: *pag_num   / Context.ctx
page_list=""					# Risorsa: *page_list / Context.ctx
export page_list
pwd=`pwd`
s02=$pwd/S02_OSF1
t2=mmi_collect4.t2				# F. temporaneo (anti: arg too long)
...
grep 'BASEPATH=' $MMI_LSTFIL | while read line
do
        var=`echo $line | sed -e 's/=.*//'`
        val=`echo $line | sed -e 's/.*=//'`
        eval "$var=\"$val\""
...
efn=mmi_collect4.err			# Log degli errori

date '+mmi_collect4 - Inizio %y-%m-%d %T' > $efn

#-----------------------------------------------------------------------
#	PULIZIA DIRECTORY globpages

#+ [ -n "$x" ] && echo "+ rm -f $gdn/*.bkg $gdn/*.pag $gdn/*.rtf ..."
#+X	"+ rm -f $gdn/*.bkg $gdn/*.pag $gdn/*.rtf ..."
X "rm -fr $gdn"
sleep 1
rm -fr $gdn
mkdir $gdn
...
grep '^R[[:space:]]' $lst | while read f1 f2 f3 f4 f5 ; do
	...
	D "$task $path $spec"
	ProcessTask ;
        page_num=`cat  VascoRossi.valore_di_pagenum` 
        page_list=`cat  VascoRossi.valore_di_pagelist` 
done
page_num=`cat  VascoRossi.valore_di_pagenum` 
page_list=`cat  VascoRossi.valore_di_pagelist` 
...
do
	...
	X "egrep -f mmi_collect4.egrep $rfn | sed -e \"s/./*$tsk./\" >> $ctx"
	egrep -f mmi_collect4.egrep $rfn | sed -e "s/./*$tsk./" >> $ctx
done
...
X "rm -f $egrep1 $t2 $scr"
#rm -f $egrep1 $t2 $scr

date '+mmi_collect4 - Fine   %y-%m-%d %T' >> $efn
