...
C     &I1,I2,XYU,'/5X,'&',19X,'DATI,K1,K2,IER,CNXYU,TOL)')
 666  FORMAT(6X,'SUBROUTINE MODIDAT(KI,IFUN,IBLOC1,IBLOC2,VAR,NX5,')
      WRITE(IOUT,667)
C GUAG2025
C 667  FORMAT(5X,'&',19X,'I1,I2,XYU,DATI,K1,K2,IER,CNXYU,TOL)')
C      WRITE(IOUT,'(A)')'      DIMENSION XYU(*),DATI(*),CNXYU(*),TOL(*)'
C      WRITE(IOUT,'(A)')'      INTEGER VAR (NX5,2)'
C      WRITE(IOUT,'(A)')'      MX1=NX5'
C      WRITE(IOUT,'(A)')'C****'
 ...
 667  FORMAT(5X,'&',19X,'I1,I2,XYU,DATI,K1,K2,IER,CNXYU,TOL)')
      ...
      WRITE(IOUT,'(A)')'      REAL BUFDATI(1)'
      WRITE(IOUT,'(A)')'      COMMON/BUFFER/IDBUFREG,NSKIP,INIZOFF,'
      WRITE(IOUT,'(A)')'     $  BUFDATI'
