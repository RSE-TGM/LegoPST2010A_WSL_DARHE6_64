C*********************************************************************
C       Fortran Source:             jacreg.f
C       Subsystem:              1
C       Description:
C       %created_by:    lomgr %
C       %date_created:  Wed Mar  9 17:33:48 2005 %
C
C**********************************************************************
...
C
      BLOCK DATA BDD_jacreg_f
      CHARACTER*80  RepoID
      COMMON /CM_jacreg_f / RepoID
      DATA RepoID/'@(#)1,fsrc,jacreg.f,2'/
      END
C**********************************************************************
...
C********************* TAVOLE ******************************************
     ...
     $          NUSTA(*),FJ(*),RIGA(*),IRJ(*),ICJ(*),
     $          NOBLC(MX3,2),RNI(*),AJAC(MMX5,*),IPVRS(*)
...
C
  ...
  205 CONTINUE
      K1=K1+NSY
  210 CONTINUE
      CALL JACTO(IRJ,ICJ,NZTER,NEQSIS,FJ)
C
C     STAMPA OPZIONALE
C
      CALL SSWTCH(3,LL)
 ...
 1001 FORMAT(//10X,'RIGA ',I4)
  ...
  250 CONTINUE
      J=NZTER+1
  260 L2=J-1
  270 WRITE(6,1002)(ICJ(J),FJ(J),J=L1,L2)
