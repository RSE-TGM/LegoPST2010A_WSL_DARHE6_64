C*********************************************************************
C       Fortran Source:             cercno.f
C       Subsystem:              1
C       Description:
C       %created_by:    lomgr %
C       %date_created:  Tue Aug 21 10:23:23 2001 %
C
C**********************************************************************
...
C
      BLOCK DATA BDD_cercno_f
      CHARACTER*80  RepoID
      COMMON /CM_cercno_f / RepoID
      DATA RepoID/'@(#)1,fsrc,cercno.f,2'/
      END
C**********************************************************************
...
C
      WRITE(6,1000)
 1000 FORMAT(//10X,'CONTROLLO CHE LE COSTANTI DI NORMALIZZAZIONE'
     $          /10X,'DI VARIABILI "LEGATE" SIANO UGUALI TRA LORO')
C
      KM=0
      IER=0
      ICRAC=0
...
C
   ...
   60 IF(IP(JB).GT.INOW) GOTO 70
      WRITE(6,*) '  LG - ERRORE IN CERCNO  INOW = ',INOW
      CALL LGABRT
   70 JB=JB-1
C               (N. blocco variabile INOW)
C
   80 WRITE(6,1002) NOBLC(JB,1),NOBLC(JB,2),VAR(INOW),CNXYU(INOW)
 1002 FORMAT(7X,2A4,4X,A8,6X,E12.5)
      IER=0
      ICRAC=1
  100 CONTINUE
      IF (ICRAC.EQ.1) THEN
        WRITE(6,*) ' LG - FATAL ERROR'
        CALL LGABRT
      ENDIF
      RETURN
      END
