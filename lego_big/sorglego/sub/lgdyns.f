C*********************************************************************
C       Fortran Source:             lgdyns.f
C       Subsystem:              1
C       Description:
C       %created_by:    lomgr %
C       %date_created:  Wed Feb 23 18:48:34 2005 %
C
C**********************************************************************
...
C
      BLOCK DATA BDD_lgdyns_f
      CHARACTER*80  RepoID
      COMMON /CM_lgdyns_f / RepoID
      DATA RepoID/'@(#)1,fsrc,lgdyns.f,2'/
      END
C**********************************************************************
...
C
     ...
     $            TSTEP,IFINE,T_PROC,JAC,ITERT)
C
C   IPR restituita a -1 anziche' 2 significa che e' stato caricato
C   uno snapshot da parte dello schedulatore. Si procede quindi a
C   forzare il calcolo di uno Jacobiano per evitare indeterminismo.
C   Spelta e Villa posero.
C
      IF (IPR.EQ.-1) THEN
         JACSI=1
      ELSE
