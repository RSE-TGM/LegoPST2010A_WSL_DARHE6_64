 ...
 *
 * Loic- 2003.05.08
 *  Fixed portability issue on Alpha (the field seq in hdrp->seq is an
 *  unsigned int on Alpha, adapted the format string for 
 *  RPC__DG_ACT_SEQ_SPRINTF
 *
 * Loic- 2004.12.10
...
#ifdef _DCE_PROTO_
(
    rpc_dg_pkt_hdr_p_t hdrp __attribute__((__unused__))
)
#else
(hdrp)
rpc_dg_pkt_hdr_p_t hdrp;
...
#ifdef _DCE_PROTO_
(
    rpc_dg_ptype_t ptype __attribute__((__unused__))
)
#else
(ptype)
