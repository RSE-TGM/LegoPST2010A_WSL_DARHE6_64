 ...
 *
 * Loic- 2003.05.08
 *   Fixed portability issue on Alpha (the AUTH_INFO_HASH macro casted a 
 *   pointer, which is 64 bits long on Alpha, to a 32 bits)
 *
 * Loic- 2004.12.10
...
#ifdef _DCE_PROTO_
(
    pointer_t junk __attribute__((__unused__))
)
#else
(junk)
pointer_t junk;
