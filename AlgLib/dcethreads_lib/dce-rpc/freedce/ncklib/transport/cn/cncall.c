 ...
 * Loic- 05.05.03: 
 *
 *  Fixed warning issued by gcc (it claimed that the variable cancel_timeout 
 *  seemed unitialized, which was wrong)
 *
 **************************************************************************/
...
#endif
{
    ...
    rpc_clock_t             cancel_timeout = 0; /*init to keep gcc happy*/
    unsigned32              temp_st;
...
(
  rpc_call_rep_p_t        call_r,
  unsigned32              *st __attribute__((__unused__))
)
#else
(call_r, st)
...
#ifdef _DCE_PROTO_
(
  rpc_cn_call_rep_p_t call_rep __attribute__((__unused__))
)
#else
(call_rep)
           ...
           (!has_timed_out))
    {
        TRY
        {
            ...
            else
            {
                ...
                pthd4_get_expiration_np ((struct timespec *) &zero_delta, 
