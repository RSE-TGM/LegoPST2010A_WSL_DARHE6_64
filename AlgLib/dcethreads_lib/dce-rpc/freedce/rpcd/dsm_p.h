 ...
 *
 * Loic- 2003.05.09 
 *   Fixed portability issue on Alpha (the preheader in the block_t structure 
...
#if defined(__LP64) 
 /*
  *  PREHEADER is 32 bytes on 64 bits Architecture
  */
#define PREHEADER   32                  /* length of preheader */
#else
 /*
  * Assume an 32 bits Architecture
  */ 
...
/*  Strong assumptions are made about the size and alignments of this
    ...
    the user data.  On 32 bits architecture, it looks like: (16 bytes)
...
/* ======================================================================
 * IMPORTANT NOTE FOR 64 BITS ARCHITECTURE (Alpha, x86_64)
 * ======================================================================
 *
 * On LP64, the preheader structure is 32 bytes long (16 for a 32 bits 
 * architecture). Strictly speaking, we could have aligned the preheader
 * on 24 bytes, this should work since the 8-bytes alignement requirement
 * is fullfiled. Nevertheless, I prefer 32 bytes, since 32 has a more
 * natural relationship to 16 as to 24. This could avoid potential bugs... 
 *
 * Of course, one could have a try with 24 bytes to see if it works. 
 *
 * Loic.
 */
...
#if defined(__LP64)
  /* 
   *  make sure that things is right padded on 64 bits Architecture.
   */
    unsigned char   unused[6];  /* preheader ends here */
#else
  /*
