 ...
 *
 * Loic- 2003.05.08 
 *   Fixed portability issue on Alpha (debug info printed on unsigned32 
 *   using the %l length modifier)
 *
 * Loic- 2004.12.10
...
error_status_t      *status;
{
    ...
    *num_ents = 0;
    ...
    if (dflag)
      /* make sure we print things right on 64-bit machines (alpha, x86_64) */
#if defined(__LP64)
      printf("ept_lookup  entry_handle %p  *entry_handle %p  *num_ents %u\n", 
...
error_status_t      *status;
{
 ...
 /* make sure we print things right on 64-bit machines (alpha, x86_64) */
#if defined(__LP64)
      printf("ept_map  entry_handle %p  *entry_handle %p  *num_towers %u\n", 
