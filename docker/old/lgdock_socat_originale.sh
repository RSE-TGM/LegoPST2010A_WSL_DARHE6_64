...
docker run --rm -it \
    ...
    bash -c "
        ...
        DEFAULTS_DIR="$USER_HOME_IN_CONTAINER/defaults"

        echo '=== Configurazione utente dinamica (UID: $HOST_USER_ID, GID: $HOST_GROUP_ID, Utente: $HOST_USERNAME defaults: $USER_HOME_IN_CONTAINER) ==='

        # Check ed eventuale creazione della directory 'defaults' (esempio)
        echo "=== Controllo della directory: $DEFAULTS_DIR ==="
        if [[ -d "$DEFAULTS_DIR" ]]; then
            echo "La directory '$DEFAULTS_DIR' esiste già."
        else
            echo "La directory '$DEFAULTS_DIR' NON esiste."
            # Se volessi crearla, potresti farlo qui
