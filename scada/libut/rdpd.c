 ...
 *						 in cui � gi� presente una stringa da modificare
 *				maxlun = massima lunghezza ammissibile per la stringa in ingresso.
 *							se maxlun � settato a 0 viene acquisito un singolo carattere
 *							senza conferma con return e senza visualizzazione.
 *							Il carattere � posto in stringa[0]. 
 ...
 *					- normale :(mod=0) non vi � una stringa di partenza.
...
int rdpd(xin,yin,buff,maxlun,mod)
...
static char ins=0;	/* flag che indica se si � in modalit� INS(=1) o normale (=0) */
short s;
short car;
	...
	if(car!=0)	 		
		...
		switch(car)
			...
			default:
				if (car>31 && car<123)
					{
					if(car>96) car-=32;		 	/* se il carattere � minuscolo lo converto in maiuscolo */
