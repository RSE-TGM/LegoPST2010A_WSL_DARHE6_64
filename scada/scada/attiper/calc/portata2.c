...
/*************************************************************
	Modulo: PORTATA2.C
        ...
        .  La pressione da atmosfere a Kg/cm2
        .  La pressione differenziale da pollici a millimetri
        ...
        .  La temperatura (in gradi Kelvin) e' uguale a zero
           oppure la pressione (in Kg/cm2) e' uguale a zero,
           oppure la pressione differenziale e' uguale a zero.
...
if( tab->corrente->p.cont_err)
...
else
 {
  ...
  val[1]=val[1] * 1.033  ;      /*press da atmosfere a Kg/cm2 */
  val[0]=val[0] * 25.4   ;      /*Dp da pollici in millimetri */

  polin= x0 + x1*val[2] + x2*val[1] + x3*val[2]*val[2] +
         x4*val[1]*val[1] + x5*val[2]*val[1];

  if( (val[2]*val[1]*polin) != 0 )
    if((radice=val[0]/(val[1]*val[2]*polin)) >= 0)
      {
        portata=(double)tab->corrente->p.cost1 * (val[1] -
          ((double)tab->corrente->p.cost2 * val[0])/(double)10000.0)*sqrt(radice);
