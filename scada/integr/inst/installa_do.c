/*
   modulo installa_do.c
   tipo 
   release 1.8
   data 6/14/96
   reserved @(#)installa_do.c	1.8
*/
/*
...
int crea_do()
{
return(scrivi_header(path_output,"editdo",editdo,NUM_CAMPI_EDITDO));
}
...
int installa_do(
	...
	int Azione,    // Tipo di Azione -1 imp, 0 a zero , 1 a uno
...
RECORD record;
char riga[50];
extern int indice_fisico_do;
static int prog = 0;

// Aggiorna tabella editnomi.txt
installa_nomi(Sigla,"DO",prog++,g1tipdo);

// inizializza il record
if(init_record(&record,editdo,NUM_CAMPI_EDITDO)!=1)
	errore("init_record");
...
// inserisce l'azione del comando
if(ins_campo_int("Azione",Azione,&record)!=1)
        errore("Azione");
...
if(ins_campo_int("IDscheda",indice_fisico_do,&record)!=1)
        errore("IDscheda");
...
if(scrivi_record(path_output,"editdo",&record)!=1)
