...
int warn_flg;
{
    ...
    if(sqlerr_code == RTERROR)   /* Exception is a run-time error */
        {
 ...
 *
 * This function handles exceptions as follows:
 *   run-time errors - continue execution, returning SQLCODE (<0)
...
int warn_flg;
{
    ...
    if(sqlerr_code == RTERROR)  /* if run-time error, return SQLCODE */
        sqlerr_code = sqlcode;
