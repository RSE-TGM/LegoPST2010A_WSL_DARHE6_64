...
*
*			Procedura ctrloop
*	Esegue controlli e decodifica per l' istruzione  LOOP
*
*********************************************************************/
...
/* Inizio controllo e decodifica istruzione LOOP */

togliblank(linea_cor,0);	// pulisce la linea di codice dai blank

M=5;             			  // si posiziona dopo la parola chiave LOOP
...
j=POS2;
j+=sprintf(&stringa[j],"%s",altre_istr[codice]);  /* inserisco LOOP */

scrivisufile(fdint,stringa,j+1,&righeint);

/* Verifico se alla parola chiave LOOP non segue la lista argomenti */

if( linea_cor[M-1] == 0x00 ) {

/* Istruzione LOOP non seguita dalla lista argomenti */
...
/* Verifica che la parola chiave LOOP sia seguita da (" altrimenti
...
/* Individua se Š arrivato alla fine istruzione LOOP */
