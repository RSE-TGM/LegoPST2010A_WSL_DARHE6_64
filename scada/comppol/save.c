...
/* alla ricerca del carattere <;> di definizione dei commenti     */
 ...
 /* Trascuro nella ricerca il carattere blank e il carattere tab 0x09 */
		...
		for(ret=0; ret<DIMFILE && *p>0x20 && *p< 0x7a && *p!=VIRGOLA; ret++,p++)
			nomefile[ret]=*p;
...
/* Inserisco il carattere nullo di terminazione del nome file di editing */
		...
		if ( opzenvincl == 1 ) {
			...
			/* opzione di set ambiente per INCLUDE selezionata */

			ier = ctreninc(nomefile);

			if ( ier == 1 ) {

				/* Concatenamento di nomefile con il path di ambiente */

					memcpy(appconc,envincl,NBYMOD);

					lstrconc = strlen(appconc);

					if ( (lstrconc + ret) >= (DIMFILE +1) ) {

						/* superamento dimensioni variabile nomefile */

						stp0_err(78,1,nomefile,nommod);

						/* Interrompo la compilazione :
						   - File di editing non specificato o non esistente */

		   			/* Cancello tutti i file temporanei prodotti dal compilatore */
					...
					strcat(appconc,nomefile);

					lstrconc = strlen(appconc);

					memcpy(nomefile,appconc,lstrconc);

					nomefile[lstrconc] = 0x00;

			}

		}

/* Salvo nella variabile di appoggio il nome del file di editing */
...
/* Verifico esistenza del file di editing e lo apro se esiste */

		if((ret=creaedt(nomefile,0,-1,-1))==-2)  {
			stp0_err(50,1,nomefile,nommod);
 ...
 /* Determino il nome del record e lo inserisco in nomerec */

		while(*p!=VIRGOLA && *p!=0x0a)
			p++;

		if(*p==0x0a) {

		/* Manca il nome del record su file di editing */

			stp0_err(49,1,nomefile,nommod);

			/* Interrompo la compilazione :
			   - Nome del record su file di editing non specificato */
         ...
         /* Incremento il puntatore al primo carattere dopo la virgola */

			p++;
			while(*p==BLANK)
				p++;

			for(ret=0; ret<10 && *p>0x20 && *p< 0x7a; ret++,p++)
				nomerec[ret]= toupper(*p);

			/* Salvo nella variabile di appoggio nomerec */

         strncpy(&appdizio[40],nomerec,ret);

			for(;ret<10;ret++)
				nomerec[ret]=0x00;

	/* Inserisco i caratteri terminatori di estensione "IN" */
			sprintf(&nomerec[ret],"%s","IN");

		 }

/* Verifico esistenza del record all' interno del file di editing indicato */
/* determino il nome del file su cui inserire il contenuto del record */

		ret=sprintf(nomefile,"%d",(*numfile)++);
		strcat(nomefile,".TMP");

/* apro il file in scrittura ed in modo BINARY, dato che la EDTFILE vede
   i file aperti in binario */

		fploc=fopen(nomefile,"w+b");
		ret=fileno(fploc);
		memcpy(nomereci,nomerec,12);
		if((ret=edtfile(nomereci,fileno(fploc)))==-1) {

			   stp0_err(51,1,nomerec,nommod);

			   /* Interrompo la compilazione :
			     - Record su file di editing non esistente */
...
/* Chiudo il file riaprendolo in lettura in modo testo, per poter leggere
   una riga alla volta */
