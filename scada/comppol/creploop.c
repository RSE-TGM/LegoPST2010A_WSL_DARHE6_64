...
*
*			Procedura creploop
*	Esegue controlli e decodifica per l' istruzione  REPLOOP
*
*********************************************************************/
...
/* Inizio controllo e decodifica istruzione REPLOOP */

togliblank(linea_cor,0);	// pulisce la linea di codice dai blank

M=8;             			   // si posiziona dopo la parola chiave REPLOOP
...
j=POS2;
j+=sprintf(&stringa[j],"%s",altre_istr[codice]);  /* inserisco REPLOOP */

scrivisufile(fdint,stringa,j+1,&righeint);

/* Verifico se alla parola chiave REPLOOP non segue la lista argomenti */

if( linea_cor[M-1] == 0x00 ) {

/* Istruzione REPLOOP non seguita dalla lista argomenti */
/* segnalo errore di sintassi                           */
...
/* Verifica che la parola chiave REPLOOP sia seguita da (" altrimenti
...
/* Ciclo di analisi lista di argomenti istruzione LOOP */
...
/* Individua se Š arrivato alla fine istruzione REPLOOP */
