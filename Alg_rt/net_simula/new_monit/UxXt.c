static char sccsid[] = "@(#)44	1.1  src/dtuimx/ibmr2/uimxdir/config/UxXt.c, uimx, dtuimx411, 9432A411a 8/8/94 19:36:46";
/*
...
*/
/*---------------------------------------------------------------------
 * $Date: 1994/07/29 14:29:41 $             $Revision: 2.22 $
 *---------------------------------------------------------------------
 * 
 *
...
#if defined(sun4) && !defined(_NO_PROTO)
...
#endif	/* sun4 && !_NO_PROTO */

static	XContext	xcontext_id = 0;
static	XContext	xadapter_id = 0;
...
int	UxPutContext( wgt, context )
	...
	void*		context;
#else
int	UxPutContext( Widget wgt, void* context )
#endif
{
	int		status;
...
RETURN:		void *			- the context pointer
...
-----------------------------------------------------------------------------*/
#ifdef _NO_PROTO
void *		UxGetContext( wgt )
		Widget	wgt;
#else
void *		UxGetContext( Widget wgt )
#endif
{
	int		status;
	void *		context;
...
#ifdef _NO_PROTO
char *UxConcatStrings(str1, str2)
char *str1, *str2;
#else
char *UxConcatStrings(char *str1, char *str2)
#endif
{
	...
	if (buf != NULL)
 		UxFree (buf);
	buf = (char *)UxMalloc (strlen(str1) + strlen(str2) + 1);
	strcpy (buf, str1);
	strcat (buf, str2);
        return buf;
}
...
/*------------------------------------------------------------------------
 * NAME: UxAdapterSwidget
 * 	<Make a component wrapping around the supplied Widget>
 *
 * INPUT:	Widget w	--	the actual widget we're proxying
 *		swidget parent	--	the parent swidget on which 
 *					we're built.
 *		char*  name	--	my name (probably the widget's)
 *		int    clsCode  --	component interface class code
 *		void*  cmpntRef	--	user reference field
 *		void*  context	--	of the interface that made me
 *				--	Question is, should this be suppled?
 * RETURNS:
 *	The widget argument, with the component features attached.
 *	Remember, swidget means Widget in Xt-mode code.  There is 
 *	no new object to create.  Simply connect the Widget to its
 *	context, class ID, and component reference.
 *------------------------------------------------------------------------*/

#ifdef _NO_PROTO
swidget	UxAdapterSwidget (w, _parent, _name, clsCode, cmpntRef, context)
	Widget w;
	swidget _parent;
	char*	_name;
	int	clsCode;
...
#endif /* _NO_PROTO */
{
	swidget parent = _parent;
	char*	name = _name;

	if (context) { 
		UxPutContext(w, context);
	} else {
		UxNewContext(0, 0);	/* Clear Last context flag */
	}
	if (clsCode != -1) {
		UxPutClassCode(w, clsCode);
	}
	if (cmpntRef) {
		UxPutComponentRef(w, cmpntRef);
	}
	return (swidget) w;
}

/*------------------------------------------------------------------------
 * NAME: UX Get/Put ComponentRef
 * 	<Bind a component reference object to the Widget.> 
 * INPUT:	Adapter swidget
 *		void* user reference data
 * EXT REFERENCES:	UxTopLevel, xadapter_id
 *------------------------------------------------------------------------*/

#ifdef _NO_PROTO
void*	UxGetComponentRef(wgt)
	swidget wgt;
#else
void*	UxGetComponentRef(swidget wgt)
#endif /* _NO_PROTO */
{
	char* context = NULL;

	if (xadapter_id) {
		XFindContext( XtDisplay( UxTopLevel ), 
		       	(Window) wgt, 
		       	xadapter_id, 
		       	&context );
	}
...
#ifdef _NO_PROTO
void	UxPutComponentRef(wgt, ref)
	swidget wgt;
	void    *ref;
#else
void	UxPutComponentRef(swidget wgt, void *ref)
#endif /* _NO_PROTO */
{
	if ( xadapter_id == 0 ) {
		xadapter_id = XUniqueContext();
	}
	XSaveContext( XtDisplay(UxTopLevel), (Window) wgt, xadapter_id, 
			       (char*) ref );
}
