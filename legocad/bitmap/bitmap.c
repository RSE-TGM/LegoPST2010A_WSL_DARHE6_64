...
*       Description:
*       %created_by:    ciccotel %
*       %date_created:  Thu Nov  7 12:19:59 2002 %
*
**********************************************************************/
#ifndef lint
static char *_csrc = "@(#) %filespec: bitmap.c-2.1.1 %  (%full_filespec: bitmap.c-2.1.1:csrc:1 %)";
#endif
/*
        Variabile per identificazione della versione
*/
static char SccsID[] = "@(#)bitmap.c	1.5\t3/20/96";
/*
...
/* 
 * This is the bitmap editor distributed by MIT with X Version 11
 * Release 2. The only modifications made to this program for VMS are 
 * the addition of this comment block and the removal of Unix specific 
 * file handling code (see #ifndef VMS sections).
 *
 ...
 *
 * BITMAP 1 "1 March 1988" "X Version 11"
 *
 * NAME
 *
 ...
 * DESCRIPTION
 *
 *     Bitmap lets you interactively create bitmaps, or edit previously
 *     created bitmaps.  A bitmap is simply a rectangular array of 0 and 1
 *     bits.  The X Window System uses bitmaps in defining clipping
 *     regions, cursor shapes, icon shapes, and tile and stipple patterns.
 *     
 *     When you run bitmap, you are given a magnified version of the
 *     bitmap, in which each bit is shown as a large square, as if it were
 *     a piece of graph paper. The pointer can be used to set, clear, or
 *     invert individual squares, and to invoke commands to set, clear or
 *     invert larger rectangular areas of the bitmap.  Other commands may
 *     be used to move or copy rectangular areas from one part of the
 *     bitmap to another, and to define a `hot spot' (a special single
 *     point on the bitmap, which is useful when the bitmap is used as an X
 *     cursor).
 *     
 *     The output of the bitmap program is a small C code fragment.  By
 *     #include'ing such a program fragment in your program, you can easily
 *     declare the size and contents of cursors, icons, and other bitmaps
 *     that your program creates to deal with the X Window System.
 *     
 * OPTIONS
 *     
 *   -help
 *     This option (or any other unsupported option) will cause a brief 
 *     description of the allowable options and paramters to be printed.
 *
 *   -display display
 ...
 *     not be drawn using dashed lines.  Although dashed lines are prettier 
 *     than solid lines, on some servers they are significantly slower.
 *
 *   -bw number
 ...
 *
 *     When  bitmap starts, it first tries to read the specified file (see
 *     FILE FORMAT). If the file already exists, it creates a window
 *     containing a grid of the appropriate dimensions.
 * 
 *     If the file does not exist, bitmap will create a window for a
 *     bitmap of the size specified by WIDTHxHEIGHT (e.g. 7x9, 13x21). The 
 *     bitmap will start out empty.  If  WIDTHxHEIGHT  is not specified
 *     specified either on the command line or in the  "Dimensions" X
 *     Default, 16x16 will be assumed.
 *
 *     The window that bitmap creates has four parts.  The largest section
 *     is the checkerboard grid, which is a magnified version of the bitmap
 *     you are editing.  At the upper right is a set of commands that you
 *     can invoke with any pointer button.  Below the commands is an
 *     "actual size" picture of the bitmap you are editing;  below that is
 *     an inverted version of the same bitmap.  Each time you alter the
 *     image in the grid, the change will be reflected in the actual-size
 *     versions of the bitmap.
 *     
 *     If you use a window manager to make the bitmap window larger or
 *     smaller, the grid squares will automatically get larger or smaller
 *     as well.
 *     
 * COMMANDS
 *     
 ...
 *     
 *   Clear All
 *     clears all squares in the bitmap.  This is irreversible, so invoke 
 *     it with care.
 *     
 *   Set All
 *     sets all squares in the bitmap.  This is irreversible, so invoke it 
 *     with care.
 *     
 *   Invert All
 ...
 *     
 *   Clear Area
 *     clears a rectangular area of the bitmap.  After you click over this
 *     command, the cursor turns into an `upper-left corner'.  Press any
 *     pointer button over the upper-left corner of the area you want to
 *     invert, hold the button down while moving the pointer to the
 *     lower-right corner of the area you want to invert, and then let the
 *     button up.
 *     
 *     While you are holding down the button, the selected area will be
 *     covered with X's, and the cursor will change to a `lower-right
 *     corner'. If you now wish to abort the command without clearing an
 *     area, either press another pointer button, move the cursor outside
 *     the grid, or move the cursor to the left of or above the upper-left
 *     corner.
 *     
 *   Set Area
 ...
 *     First, you select the rectangle to be copied, in the manner
 *     described above under Clear Area above.  Then, the cursor will
 *     change to an "upper-left corner".  When you press a pointer button,
 *     a destination rectangle will overlay the grid;  moving the pointer
 *     while holding down the button will move this destination rectangle. 
 *     The copy will occur when you let up the button.  To cancel the copy,
 *     move the pointer outside the grid and then let up the button.
 *     
 *   Move Area
 *     works identically to Copy Area, except that it clears the source 
 *     rectangle after copying to the destination.
 *     
 *   Line
 *     will draw a line between two points.
 *
 *   Overlay Area
 *     works identically to Copy Area, except that it does a binary OR
 ...
 *     using your bitmap as a cursor, the hot spot indicates which point on
 *     the bitmap is the "actual" location of the cursor.  For instance, if
 *     your cursor is an arrow, the hot spot should be the tip of the
 *     arrow;  if your cursor is a cross, the hot spot should be where the
 ...
 *     
 *   Write Output
 *     writes the current bitmap value to the file specified in the
 *     original command line. [In the VMS version, any file open errors
 *     are reported on sys$error.]
 *     
 *   Quit
 *     exits the bitmap program.  If you have edited the bitmap and have 
 *     not invoked Write Output, or you have edited it since the last time 
 *     you invoked Write Output, a dialog window will appear, asking if you 
 *     want to save changes before quitting. ``Yes'' does a ``Write Output'' 
 *     before exiting;  ``No'' just exits, losing the edits;  ``Cancel'' 
 *     means you decided not to quit after all.
 *     
 * FILE FORMAT
 *     
 *     Bitmap reads and writes files in the following format, which is
 ...
 *        0x10, 0x00};
 *     
 *     The variables ending with _x_hot and _y_hot are optional; they will 
 *     be present only if a hot spot has been defined for this bitmap.  
 *     The other variables must be present.
 *     
 *     The name portion of the five variables will be derived from the name
 *     of the file that you specified on the original command line by
 ...
 *     /usr/include/bitmaps/cross.bitmap will produce a file with variable
 *     names cross_width, cross_height, and cross_bits (and cross_x_hot
 *     and cross_y_hot if a hot spot is defined).
 *     
 *     It's easy to define a bitmap or cursor in an X program by simply
 *     #include'ing a bitmap file and referring to its variables.  For
 *     instance, to use a cursor defined in the files this.cursor and
 *     this_mask.cursor, one simply writes
 *     
 *     #include "this.cursor"
 ...
 *     
 *     An X program can also read a bitmap file at runtime by using the
 ...
 *     
 *     For backward compatibility with X10, bitmap can also read in a file
 ...
 *     in such a file, bitmap will always write the file back out
 *     in standard X11 format.
 *     
 * X DEFAULTS
 *     
 *     The bitmap program uses the routine XGetDefault to read defaults, 
 ...
 *     commands.   If a highlight color is not given, then bitmap
 *     will highlight by inverting.  This option is useful only on 
 *     color displays.
 *
 *   Mouse
 ...
 *     Any of these conditions aborts bitmap before it can create its window.
 *     
 ...
 *     
 *     You invoked bitmap with no command line arguments.  You must give a
 ...
 *     
 *     The following messages may be printed after bitmap
 *     creates its window:
 *       
 *       ``bitmap: Unrecognized variable name in file filename''
 *     bitmap encountered a variable ending in something other than
 *     _x_hot, _y_hot, _width, _height while parsing the input file.  It 
 *     will ignore this variable and continue parsing the file.
 *     
...
#ifndef lint
static char *rcsid_bitmap_c = "$Header: bitmap.c,v 1.26 88/02/19 18:53:24 jim Exp $";
#endif
#include <stdio.h>
...
#define MAX_LINE 81
  ...
  for (;;) {
    ...
    /*
     * Figure out how many extra bytes there are at the end of each
     * line of input.  Since V11 bitmaps are padded out to 8 bits,
     * only old V10 formats with 1-8 bits of data in the last word will
     * have any padding.  Note that if there is padding, the number of bytes 
     * read in will be greater than the number of bytes in the image.
     */
    ...
    bytes_per_line_input = image.bytes_per_line + padding;
    
    raster_length =  image.bytes_per_line * image.height;
    bytes_of_input = bytes_per_line_input * image.height;
    ...
    if (version10p)
      for (bytes=0, ptr=image.data; bytes<bytes_of_input; (bytes += 2)) {
	if (fscanf(stream, " 0x%x%*[,}]%*[ \n]", &value) != 1) {
	  cleanup(image.data, stream);
	  return(BitmapFileInvalid);
	}
	*(ptr++) = value & 0xff;
	if (!padding || ((bytes+2) % bytes_per_line_input))
	  *(ptr++) = value >> 8;
      }
    else
      for (bytes=0, ptr=image.data; bytes<bytes_of_input; bytes++, ptr++) {
	if (fscanf(stream, " 0x%x%*[,}]%*[ \n]", &value) != 1) {
	  cleanup(image.data, stream);
	  return(BitmapFileInvalid);
	}
...
#else
  perror("bitmap can't write file");
  return (FALSE);
#endif
  }
