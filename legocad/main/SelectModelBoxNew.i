...
/*\
   modulo SelectModelBoxNew.i\
   tipo \
   release 1.21\
   data 5/12/95\
   reserved @(#)SelectModelBoxNew.i	1.21\
*/\
/*\
...
*SelectModelBox.auxdecl: refresh_lista(swidget w)\
{\
Arg arg[2];\
XmString *modelli;\
char **str_modelli;\
...
/*::\
   WDescr = create_form4(); \
   UxPopupInterface(WDescr,no_grab); \
::*/\
   WDescr = create_editText(UxWidget,EDIT_DESCR,NULL,NULL);\
   attesa_oknew = True; \
}\
\
\
...
/*::\
   ...
   while( attesa_oknew == True)\
   {\
      while (XCheckIfEvent(UxDisplay, &evento, predproc))\
