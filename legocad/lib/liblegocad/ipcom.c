...
int send_message( fd, appl, codice, mesg, mesg_len )
int  fd;
char *appl;
int  codice;
char *mesg;
...
int recv_message(fd, messaggio, hmsg)
int fd;
char *messaggio;
header_mesg *hmsg;
{
   ...
   if (read_pipe( fd, (char*)hmsg, sizeof(header_mesg)) == NOT_OK)
   {
      hmsg->codice = -1;
      return(NOT_OK);
   }
