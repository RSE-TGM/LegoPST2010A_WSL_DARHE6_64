/**********************************************************************
*
*       C Source:               genera_ftn.c
*       Subsystem:              1
*       Description:
*       %created_by:    famgr %
*       %date_created:  Tue Apr 30 12:18:38 1996 %
*
**********************************************************************/
#ifndef lint
static char *_csrc = "@(#) %filespec: genera_ftn.c,2 %  (%full_filespec: 1,csrc,genera_ftn.c,2 %)";
#endif
/*
        Variabile per identificazione della versione
*/
static char SccsID[] = "@(#)genera_ftn.c	2.25\t1/29/96";
/*
...
int leggi_file_interfaccia()
{
   ...
   int i, j, num_st, num_alg, num_in, num_dati, numero_variabili;
   int flag = 0;

   componi_file_modulo(nome_file_interfaccia,nome_modulo,FILE_IFACE,LIBUT,
                       LIB_UTENTE);
   ...
   if (ifile_header( fp_iface, &num_st, &num_alg, &num_in, &num_dati) != 0)
   {
      num_st = 0;
      num_alg = 0;
      num_in = 0;
   ...
   numero_variabili = num_st+num_alg+num_in;
   
/* allocazione della memoria */
   variabili = (StructVars *) calloc(numero_variabili,sizeof(StructVars));
   dati_geom = (StructDati *) calloc(num_dati,sizeof(StructDati));

/* Azzera lo jacobiano */
   for (i=0 ; i<num_st+num_alg ;i++)
      for (j=0 ; j<num_st+num_alg+num_in ;j++)
      {
         jactoggstruct[i][j].jac_yes = False;
         jactoggstruct[i][j].stringa = NULL;
      }

   ifile_variabili( fp_iface, variabili, num_st, num_alg, num_in );
   ifile_dati_geometrici( fp_iface, dati_geom, num_dati );
   ifile_jacobiano( fp_iface, &jacobian_type, jactoggstruct, &num_yes_toggle );
   ifile_subI2( fp_iface,&str_dichI2, &str_codI2 );
   ifile_subJC( fp_iface,&str_dichJC,&str_codJC,jactoggstruct,num_st+num_alg,
		         numero_variabili );
   ifile_subMOD( fp_iface,&str_dichMOD,&str_codMOD,str_resMOD,num_st+num_alg );
   ifile_subD1( fp_iface, str_signeq, str_uniteq, str_cosnor, num_st+num_alg );

   fclose(fp_iface);

   alloca_text_widget(UxGetWidget(nmod_bboard),num_st,num_alg,num_in,num_dati);

/* Riempimento dei text-widget relativi alle variabili e ai dati geometrici */
   for ( i=0 ; i<num_st ; i++ )
   {
       XmTextSetString(wvar_stato[i].text_descr, variabili[i].descr);
       XmTextSetString(wvar_stato[i].text_simbolo, variabili[i].nome);
       XmTextSetString(wvar_stato[i].text_valore, variabili[i].norm);
   }

   for ( i=0 ; i<num_alg ; i++ )
   {
       XmTextSetString(wvar_algebriche[i].text_descr, 
                       variabili[i+num_st].descr);
       XmTextSetString(wvar_algebriche[i].text_simbolo,
                       variabili[i+num_st].nome);
       XmTextSetString(wvar_algebriche[i].text_valore,
                       variabili[i+num_st].norm);
   }

   for ( i=0 ; i<num_in ; i++ )
   {
       XmTextSetString(wvar_ingresso[i].text_descr, 
                       variabili[i+num_st+num_alg].descr);
       XmTextSetString(wvar_ingresso[i].text_simbolo,
                       variabili[i+num_st+num_alg].nome);
       XmTextSetString(wvar_ingresso[i].text_valore,
                       variabili[i+num_st+num_alg].norm);
   }

   for ( i=0 ; i<num_dati ; i++ )
   {
       XmTextSetString(wdati_geometrici[i].text_dato, dati_geom[i].nome);
       XmTextSetString(wdati_geometrici[i].text_sigla, dati_geom[i].sigla);
   }
   return(flag);
}
...
char *nome_modulo, *titolo_esteso; 
{
   ...
   componi_file_modulo(nome_file_fortran,nome_modulo,FILE_FTN,LIBUT,LIB_UTENTE);
